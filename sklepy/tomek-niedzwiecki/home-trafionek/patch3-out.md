...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .product-media .card-vid.is-playing {
      opacity: 1;
    }

    @media (hover: hover) {
      .product-card:not(.has-vid):hover .product-image-secondary,
      .product-card:not(.has-vid):focus-visible .product-image-secondary {
        opacity: 1;
      }
    }
...
<body>
      ...
      <div class="wrap">
        ...
        <a class="product-card reveal" href="{{CARD_URL}}" aria-label="{{CARD_CTA}}">
          <div class="product-media">
            ...
            <!--IFVID-->
            <video class="card-vid" muted playsinline loop preload="none" poster="{{CARD_IMG}}" aria-hidden="true" tabindex="-1">
              <source data-src="{{CARD_VID_WEBM}}" type="video/webm">
              <source data-src="{{CARD_VID_MP4}}" type="video/mp4">
            </video>
            <!--/IFVID-->
          </div>
  ...
  <script>
    (function () {
      ...
      var cardVideos = Array.prototype.slice.call(document.querySelectorAll(".card-vid"));

      if (!reduceMotion && cardVideos.length && "IntersectionObserver" in window) {
        function hydrateCardVideo(video) {
        ...
        function playCardVideo(video) {
          hydrateCardVideo(video);

          if (video.dataset.unavailable === "true") return;
          ...
          if (playPromise && typeof playPromise.then === "function") {
            playPromise
              .then(function () {
                ...
                var card = video.closest(".product-card");
                if (card) {
                  card.classList.add("has-vid");
                }
              })
              .catch(function () {});
          }
        ...
        var cardVideoObserver = new IntersectionObserver(function (entries) {
          entries.forEach(function (entry) {
