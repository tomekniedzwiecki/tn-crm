Dostajesz kompletny template.html strony głównej sklepu Trafionek (poniżej). Wprowadź DOKŁADNIE dwie poprawki jakościowe, niczego innego nie zmieniaj (markery <!--CARDS:START/END-->, <!--CARD-TEMPLATE ... CARD-TEMPLATE-->, <!--ITEMLIST:START/END-->, placeholdery {{...}} — NIETYKALNE):

1. INTRO desktop (>=980px): prawa ~45% sekcji intro jest pustym kremem. Dodaj po prawej DUŻĄ sygnaturę marki: obraz https://yxmavwkwnfuphjqbelws.supabase.co/storage/v1/object/public/attachments/bud-assets/parasol-trafionek/brand/favicon-512.png (znak metki z ptaszkiem) o szerokości ~260-300px, osadzony na subtelnym kremowym medalionie (koło/squircle w bg-alt #F6EDE2, delikatny border #E7DCCD, miękki cień), lekko obrócony (-4deg) dla życia. Na mobile (<980px) ukryj ten element (display:none) — intro zostaje jak jest. Obraz z loading="lazy" alt="" aria-hidden="true", z width/height (zero CLS).
2. PAS ZAUFANIA desktop: chipy są zbyt szerokie względem krótkich etykiet (pustka w środku). Zmień kontener chipów tak, żeby chipy przylegały do treści (np. flex z justify-content:center i width:auto na chipach, gap zamiast równego grid 1fr) — na mobile bez zmian (stack).
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Fredoka:wght@500;600;700&family=Nunito:wght@400;500;600;700;800&display=swap&subset=latin-ext" rel="stylesheet">
