Dostajesz template.html strony głównej sklepu Trafionek (na dole). KOREKTA KIERUNKU (decyzja właściciela): wideo produktów ma żyć W KARTACH GALERII, NIE w hero. Wprowadź DOKŁADNIE te zmiany. Markery <!--CARDS:START/END-->, <!--CARD-TEMPLATE ... CARD-TEMPLATE-->, <!--ITEMLIST:START/END--> i placeholdery {{...}} — NIETYKALNE poza jawnie wskazanymi rozszerzeniami.

ZMIANA 1 — HERO: USUŃ rotator wideo całkowicie (markup dwóch <video>, chip-link, cały jego JS oraz markery <!--HEROVIDS:START--><!--HEROVIDS:END-->). Przywróć statyczny medalion z faviconem (fallback już jest w kodzie — ma być jedynym wizualem intro na desktopie, jak przed rotatorem: squircle bg-alt, border, cień, obrót -4deg, ukryty <980px).

ZMIANA 2 — WIDEO W KARTACH PRODUKTÓW: w CARD-TEMPLATE, w kontenerze mediów karty (position:relative, proporcja 1:1, zaokrąglenia bez zmian):
- Obraz {{CARD_IMG}} ZOSTAJE (jest posterem/fallbackiem, renderowany zawsze).
- Dodaj blok owinięty DOKŁADNIE markerami <!--IFVID--> ... <!--/IFVID--> (montaż WYCINA go, gdy produkt nie ma wideo): <video class="card-vid" muted playsinline loop preload="none" poster="{{CARD_IMG}}" aria-hidden="true" tabindex="-1"> z <source data-src="{{CARD_VID_WEBM}}" type="video/webm"> i <source data-src="{{CARD_VID_MP4}}" type="video/mp4">; position:absolute inset:0, object-fit:cover, opacity:0, transition opacity .4s.
- JS (jeden IntersectionObserver dla wszystkich .card-vid): gdy karta wchodzi w viewport → przy pierwszym wejściu przepisz data-src→src na <source> (pomiń source z pustym data-src), load(), play(); po udanym starcie odtwarzania ustaw opacity:1 (płynne przejście z postera). Gdy karta wychodzi z viewportu → pause() (oszczędność baterii). Autoplay może być odrzucony (błąd łap cicho → karta zostaje na obrazie).
- prefers-reduced-motion: reduce → wideo w kartach NIE startuje (obraz zostaje).
- HOVER-SWAP drugiego obrazu {{CARD_IMG2}}: ogranicz go do kart BEZ wideo — karta z grającym wideo nie podmienia nic na hover (dodaj np. klasę .has-vid ustawianą w JS po starcie wideo i wyłącz swap dla niej), żeby uniknąć migotania trzech warstw.
- Wideo gra też na mobile (muted+playsinline; pliki są lekkie, pionowe) — bez zmian w layoutach.
    ...
    *::after {
    ...
    @media (min-width: 980px) {
      ...
      .hero-video-chip {
        ...
        border: 1px solid var(--border);
        border-radius: var(--radius);
        box-shadow: 0 8px 20px rgba(38, 34, 30, 0.1);
        color: var(--ink);
        font-size: 14px;
        font-weight: 800;
        line-height: 1.2;
    ...
    .product-media .product-image-secondary {
      position: absolute;
      inset: 0;
      opacity: 0;
    ...
    <section class="intro" aria-labelledby="intro-title">
      <div class="wrap">
        ...
        <div class="intro-visual">
          <div class="hero-rotator" hidden>
            <video muted playsinline loop preload="none" aria-hidden="true"></video>
            <video muted playsinline loop preload="none" aria-hidden="true"></video>
            <a class="hero-video-chip" href="#"></a>
          </div>

          <div class="intro-signature" aria-hidden="true" hidden>
            <img
      ...
      <div class="wrap">
        ...
        <a class="product-card reveal" href="{{CARD_URL}}" aria-label="{{CARD_CTA}}">
          <div class="product-media">
            ...
            <img class="product-image-secondary" src="{{CARD_IMG2}}" width="720" height="720" alt="" loading="lazy" decoding="async" aria-hidden="true">
          </div>
  ...
  <script>
    (function () {
      ...
      function initHeroRotator() {
        ...
        var fallback = document.querySelector(".intro-signature");
        if (!rotator || !fallback) return;

        var motionQuery = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)");
        var desktopQuery = window.matchMedia && window.matchMedia("(min-width: 980px)");
        var sourceClips = Array.isArray(window.__HOME_VIDS) ? window.__HOME_VIDS : [];
        var clips = sourceClips.filter(function (clip) {
          return clip && (clip.mp4 || clip.webm);
        });
        ...
        var videos = Array.prototype.slice.call(rotator.querySelectorAll("video"));
        var chip = rotator.querySelector(".hero-video-chip");
        if (videos.length !== 2 || !chip) {
          showFallback();
          return;
        }
        ...
        var currentIndex = 0;
        var intervalId = null;
        var prepareTimeoutId = null;
        var initialized = false;
        var inViewport = false;

        function setChip(index) {
          var clip = clips[index];
          chip.href = clip.landing || "#";
          chip.textContent = (clip.name || "Zobacz produkt") + " →";
        }

        function loadSlot(slotIndex, clipIndex) {
          var video = videos[slotIndex];
          var clip = clips[clipIndex];

          if (video._clipIndex === clipIndex) return;
        ...
        function advance() {
          if (!inViewport || clips.length < 2) return;

          var nextSlot = activeSlot === 0 ? 1 : 0;
          var nextIndex = (currentIndex + 1) % clips.length;

          loadSlot(nextSlot, nextIndex);
          playVideo(videos[nextSlot]);

          videos[activeSlot].classList.remove("is-active");
          videos[nextSlot].classList.add("is-active");
          setChip(nextIndex);
          ...
          prepareTimeoutId = window.setTimeout(function () {
            if (!inViewport) return;

            var hiddenSlot = activeSlot === 0 ? 1 : 0;
            var followingIndex = (currentIndex + 1) % clips.length;
            loadSlot(hiddenSlot, followingIndex);
            playVideo(videos[hiddenSlot]);
          }, 850);
        }
        ...
        function start() {
          ...
          if (!initialized) {
            ...
            videos.forEach(function (video) {
              if (typeof video._clipIndex === "number") {
                playVideo(video);
              }
            });
          }

          if (clips.length > 1 && intervalId === null) {
        ...
        heroObserver.observe(rotator);
      }

      window.addEventListener("load", initHeroRotator, { once: true });
