...
**Wykonawcą:** {{WYKONAWCA_NAZWA}}, NIP {{WYKONAWCA_NIP}}, {{WYKONAWCA_ADRES}} („**Wykonawca**")
...
> **Podsumowanie podstawowych warunków** *(charakter informacyjny — wiążąca jest treść paragrafów):*
> - **Zakres Wykonawcy.** Wykonawca buduje Aplikację Partnera i prowadzi jej rozruch do pierwszych 50 stałych klientów (rozruch trwa do osiągnięcia tego progu, nie dłużej niż przez 12 miesięcy). Cena za budowę jest jednorazowa i wynosi {{CENA_NETTO}} zł netto.
> - **Prawa Partnera.** Kod i pełne autorskie prawa majątkowe (przenoszone po odbiorze MVP i zapłacie całości ceny za budowę), marka, domena, baza klientów oraz przychody należą do Partnera. Partner rozwija Aplikację według własnego wyboru.
> - **Wynagrodzenie Wykonawcy od Przychodu.** Wynagrodzenie Wykonawcy za przeniesienie praw wynosi {{FEE_PERCENT}}% Przychodu Aplikacji i jest rozliczane automatycznie, bez czynności po stronie Partnera. Bez dodatkowych opłat Wykonawca utrzymuje system płatności, dostarcza aktualizacje bezpieczeństwa oraz świadczy doradztwo strategiczne.
> - **Wykup.** Po upływie 12 miesięcy od startu Partner może w każdej chwili wykupić Udział Wykonawcy według z góry określonej formuły.
> - **Jednakowe warunki.** Warunki są jednakowe dla każdego partnera Wykonawcy — ta sama cena, ten sam procent, ta sama umowa.

---

## § 1. Przedmiot i role

1. Wykonawca zaprojektuje i zbuduje aplikację internetową opisaną w Załączniku 1 („**Aplikacja**"), uruchomi ją produkcyjnie i poprowadzi jej rozruch rynkowy do osiągnięcia pierwszych 50 stałych klientów (§ 2 pkt „50 stałych klientów"), a następnie przekaże Partnerowi samodzielne prowadzenie Aplikacji, pozostając doradcą strategicznym.
...
3. Umowa nie tworzy spółki (w szczególności spółki cywilnej — art. 860 k.c.), konsorcjum ani wspólnego przedsiębiorstwa. Strony działają jako niezależni przedsiębiorcy, każda na własny rachunek; żadna nie jest uprawniona do zaciągania zobowiązań w imieniu drugiej ani do reprezentowania jej wobec osób trzecich.
4. **Ramy rozruchu.** Rozruch rynkowy (ust. 1) Wykonawca prowadzi do osiągnięcia 50 stałych klientów, nie dłużej jednak niż przez **12 miesięcy od Startu produkcyjnego (uruchomienia sprzedaży)** *[DO POTWIERDZENIA — TOMEK; patrz notatki v4]*. Po upływie tego terminu — niezależnie od osiągniętej liczby klientów — operacyjne prowadzenie Aplikacji przechodzi na Partnera, a obowiązek prowadzenia rozruchu przez Wykonawcę wygasa; nie wpływa to na Udział (§ 6), który trwa na dotychczasowych zasadach. Koszty budżetu reklamowego kampanii rozruchowych (wydatki na media, w szczególności Meta Ads) ponosi **Partner**; Wykonawca planuje i prowadzi te kampanie w ramach rozruchu bez dodatkowego wynagrodzenia. Wydatki mediowe rozliczane są bezpośrednio z konta reklamowego Partnera, a jeżeli wyjątkowo poniósł je za Partnera Wykonawca — Partner zwraca mu je na podstawie zestawienia.
5. **Współdziałanie Partnera.** Partner współdziała przy budowie i rozruchu — dostarcza materiały, podejmuje decyzje, udziela dostępów, utrzymuje czynne konto płatności (Stripe) i zapewnia środki na budżet reklamowy rozruchu (ust. 4); brak tych środków traktuje się jak brak współdziałania. Brak wymaganego współdziałania **zawiesza bieg terminów** wykonania i odbioru (w tym terminu z § 4 ust. 1) na czas trwania przeszkody, **nie zawiesza jednak** 12-miesięcznego okresu rozruchu z ust. 4; jeżeli uniemożliwia on prowadzenie rozruchu, Wykonawca może zakończyć rozruch wcześniej — po bezskutecznym upływie 14 dni od wezwania. Po przekazaniu operacyjnego prowadzenia Aplikacji Wykonawca świadczy doradztwo strategiczne w rozsądnym wymiarze — orientacyjnie do **4 godzin konsultacji miesięcznie** *[DO POTWIERDZENIA — TOMEK]* — na zasadach § 7.
...
3. Ryzyko, że budżet nie wystarczy na wykonanie umówionej funkcjonalności, obciąża Wykonawcę — różnicę do wykonania Zakresu MVP pokrywa on z własnych środków.
...
2. **Procedura odbioru.** Po ukończeniu MVP Wykonawca zgłasza je Partnerowi do odbioru (mailowo albo z użyciem wzoru protokołu odbioru). W terminie **7 dni** od zgłoszenia Partner albo potwierdza odbiór, albo zgłasza uwagi (na piśmie lub mailem). **Brak uwag w tym terminie uważa się za dokonanie odbioru.** Uwagi zasadne w zakresie Załącznika 1 (Zakres MVP) Wykonawca usuwa i ponownie zgłasza rezultat do odbioru, przy czym 7-dniowy termin biegnie na nowo co do ponownie zgłoszonego zakresu; uwagi wykraczające poza Zakres MVP traktuje się jako zamówienie nowych funkcji (§ 3 ust. 2). Dzień dokonania odbioru (potwierdzenia albo bezskutecznego upływu terminu) jest **datą odbioru**. Wobec Partnera będącego Konsumentem albo PNPK milczący odbiór (bezskuteczny upływ terminu) ma wyłącznie znaczenie organizacyjne i nie stanowi potwierdzenia zgodności Aplikacji z umową, zrzeczenia się roszczeń ani ograniczenia ustawowej odpowiedzialności Wykonawcy.
3. **Gwarancja.** Wykonawca zapewnia **30-dniową gwarancję** usuwania błędów, liczoną **od daty odbioru** (ust. 2) — usuwanie wad polegających na niedziałaniu Aplikacji zgodnie z Zakresem MVP — a niezależnie od niej stałą opiekę i serwis opisane w § 7.

## § 5. Wynagrodzenie za budowę

1. Wynagrodzenie za budowę i wdrożenie Aplikacji oraz za rozruch wynosi **{{CENA_NETTO}} zł netto ({{CENA_BRUTTO}} zł brutto)** i jest płatne z góry, w całości, przy zawarciu umowy. Wpłacona wcześniej opłata rezerwacyjna {{REZERWACJA_KWOTA}} zł (kwota brutto) jest w pełni zaliczana na poczet tej kwoty — do zapłaty pozostaje wówczas **{{KWOTA_POZOSTALA_BRUTTO}} zł brutto**. *[DO KSIĘGOWEJ: moment powstania obowiązku VAT od zaliczki rezerwacyjnej oraz charakter opłaty rezerwacyjnej jako kaucji zwrotnej do czasu jej zaliczenia.]*
2. Kwoty netto powiększa podatek VAT według stawki obowiązującej w dniu wystawienia faktury, o ile Wykonawca jest jego podatnikiem. *[Status VAT stron oraz stawka — do potwierdzenia z księgową; patrz notatki v4.]*
3. Poza § 6 (Udział) oraz udziałem w kosztach infrastruktury po 12. miesiącu (§ 11) Wykonawca nie pobiera żadnych opłat stałych, abonamentów ani innych opłat nieprzewidzianych Umową.

## § 6. Udział — wynagrodzenie od Przychodu

1. **Udział to zmienna część ceny za przeniesienie autorskich praw majątkowych do Aplikacji (§ 8), ustalona jako procent wpływów z eksploatacji Aplikacji** (model wynagrodzenia procentowego znany prawu autorskiemu), niezależna od świadczeń bieżących Wykonawcy. Obok kwoty z § 5 Udział wynosi **{{FEE_PERCENT}}% Przychodu Aplikacji** i jest należny za każdy miesiąc kalendarzowy, w którym Aplikacja osiąga Przychód. **Udział jest ograniczony w czasie: wygasa najpóźniej z upływem 15 lat od Startu produkcyjnego (uruchomienia sprzedaży), a wcześniej — z chwilą wykupu (§ 9).** Rozruch rynkowy do 50 stałych klientów (§ 1 ust. 1) mieści się w cenie za budowę z § 5 i nie jest odrębnie wynagradzany Udziałem.
...
3. **Pobór.** Udział pobierany jest automatycznie jako opłata platformy (*application fee*) w systemie Stripe Connect (konto Wykonawcy = platforma, konto Partnera = *connected account* typu Standard; Partner jest *merchant of record*). Partner nie wykonuje żadnych przelewów. Partner utrzymuje konfigurację płatności umożliwiającą ten pobór i nie zmienia jej bez uzgodnienia. Automatyczny pobór jest wyłącznie techniką zapłaty: gdyby był niemożliwy (np. inny kanał płatności uzgodniony przez strony, zmiana zasad Stripe), Partner zapłaci Udział przelewem na podstawie faktury, w terminie 14 dni od końca miesiąca, wraz z raportem sprzedaży, a na wniosek Wykonawcy udostępni wgląd (tylko do odczytu) w dane sprzedażowe Aplikacji.
4. **Wyłączny kanał płatności.** Partner przyjmuje płatności od użytkowników Aplikacji wyłącznie przez zintegrowane z nią kanały płatności i nie kieruje ich poza system Aplikacji w celu zaniżenia Przychodu. W razie uzasadnionego podejrzenia naruszenia Przychód za dany okres ustala się szacunkowo na podstawie danych analitycznych Aplikacji (liczba użytkowników, aktywnych subskrypcji, ruch), przy czym Partner może wykazać rzeczywiste kwoty danymi z systemów płatniczych (**domniemanie wzruszalne**); wcześniej Wykonawca wzywa Partnera do zaprzestania naruszenia i rozliczenia w **terminie 14 dni**.
5. **Rozliczenie.** Okresem rozliczeniowym Udziału jest miesiąc kalendarzowy. Wykonawca wystawia zbiorczą fakturę za dany miesiąc. **Kwota Udziału jest kwotą brutto i zawiera należny podatek VAT** (spójnie z poborem *application fee* w pełnej wysokości — ust. 3). *[Moment powstania obowiązku VAT przy automatycznym poborze application fee — do potwierdzenia z księgową; patrz notatki v4.]*
6. Udział jest jednakowy dla wszystkich partnerów Wykonawcy i nie podlega negocjacji.
7. Wykonawca nie gwarantuje żadnego poziomu przychodów ani liczby klientów Aplikacji.
8. **Przychód — zasady liczenia.** Do Przychodu wlicza się kwoty brutto faktycznie otrzymane od użytkowników z eksploatacji Aplikacji, przy czym:
   (a) płatności z góry za dłuższy okres (np. roczne) zalicza się w całości do miesiąca ich otrzymania;
   (b) liczą się kwoty po rabatach — faktycznie zapłacone przez użytkownika;
   (c) prowizje i opłaty operatora płatności nie pomniejszają podstawy;
   (d) zwroty i chargebacki koryguje się w najbliższym rozliczeniu; wynik miesiąca nie schodzi poniżej zera, a nadwyżkę korekt przenosi się na kolejne okresy;
   (e) wpływy w walutach obcych przelicza się po średnim kursie NBP z ostatniego dnia miesiąca, którego dotyczą;
   (f) wlicza się wpływy podmiotów powiązanych z Partnerem z eksploatacji Aplikacji;
...
3. **Dostępność.** Wykonawca zapewnia staranne działanie w celu utrzymania ciągłości Aplikacji, jednak nie gwarantuje dostępności wyższej niż zapewniają jej dostawcy chmurowi (hosting, baza, płatności, poczta). Z serwisu wyłączone są przerwy wynikające z: siły wyższej, awarii lub ograniczeń po stronie dostawców zewnętrznych, działań lub zaniechań Partnera i jego użytkowników, ataków i incydentów bezpieczeństwa niezawinionych przez Wykonawcę oraz zaplanowanych okien serwisowych.
...
5. **Zgodność usługi cyfrowej.** Jeżeli Partner jest Konsumentem albo PNPK, w okresie świadczenia opieki i serwisu Wykonawca zapewnia zgodność Aplikacji jako usługi/treści cyfrowej z umową na zasadach rozdziału 5b UoPK, w tym dostarcza konieczne aktualizacje (w szczególności zabezpieczające). Postanowień ust. 2–4 nie interpretuje się jako wyłączenia lub ograniczenia tej ustawowej odpowiedzialności. Z chwilą wykupu (§ 9) albo zakończenia opieki i serwisu Partner staje się samodzielnym operatorem Aplikacji i przejmuje odpowiedzialność za jej dalszą zgodność wobec swoich użytkowników.
6. **Funkcje AI.** Jeżeli Aplikacja zawiera funkcje oparte na sztucznej inteligencji, obowiązki regulacyjne z nimi związane, spoczywające na operatorze Aplikacji, po przejęciu jej operacyjnego prowadzenia wykonuje Partner. Wykonawca przekazuje Partnerowi informacje o wykorzystanych modelach i ich dostawcach.

## § 8. Przeniesienie autorskich praw majątkowych
...
8. **Oświadczenia i zapewnienia Wykonawcy:**
   ...
   (b) w zakresie części Aplikacji wytworzonych z użyciem narzędzi sztucznej inteligencji Wykonawca przenosi na Partnera całość praw, jakie mu przysługują, a w odniesieniu do elementów niepodlegających ochronie prawa autorskiego (np. rezultatów pozbawionych indywidualnego wkładu twórczego człowieka) udziela Partnerowi **nieodwołalnego, bezwarunkowego i nieodpłatnego zezwolenia** na korzystanie z nich i rozporządzanie nimi bez ograniczeń oraz zobowiązuje się nie podnosić wobec Partnera żadnych roszczeń z tego tytułu;
   (c) Aplikacja może zawierać komponenty open-source oraz inne zasoby osób trzecich (biblioteki, fonty, ikony, szablony, materiały stockowe), które pozostają na swoich pierwotnych licencjach — Partner korzysta z nich na warunkach tych licencji, a nie na podstawie przeniesienia praw z ust. 2; ich wykaz (nazwa, wersja, licencja, źródło) wraz z tekstami licencji i wymaganymi notami (notices) stanowi Załącznik 4. Wykonawca zobowiązuje się nie wykorzystywać w Aplikacji komponentów o licencjach typu copyleft mogących rozciągnąć swoje warunki na własny kod Aplikacji (w szczególności AGPL, SSPL, GPL) bez uprzedniej zgody Partnera;
   (d) rezultaty prac wytworzone przez podwykonawców lub współtwórców objęte są pisemnym przeniesieniem na Wykonawcę autorskich praw majątkowych w zakresie niezbędnym do wykonania niniejszej Umowy, wraz z prawem ich dalszego przeniesienia na Partnera.
9. **Faktyczne wydanie.**
   (a) Na żądanie Partnera zgłoszone po zapłacie całości ceny za budowę Wykonawca udostępnia mu kopię repozytorium kodu źródłowego (dostęp *read-only*) — bez dostępów produkcyjnych i bez przekazania administracji infrastrukturą, które pozostają przy Wykonawcy do czasu wykupu. Umożliwia to Partnerowi wykonywanie przeniesionych praw (np. audyt, przeniesienie do innego programisty), zgodnie z celem umowy.
   (b) Pełne repozytorium kodu źródłowego wraz z dokumentacją dostępową (dostępy produkcyjne, konfiguracja) Wykonawca przekazuje Partnerowi **przy wykupie** (§ 9 ust. 2); do tego czasu administruje infrastrukturą (§ 11).
...
10. **Licencja zwrotna dla Wykonawcy.** Partner udziela Wykonawcy licencji niewyłącznej, nieodpłatnej, na czas obowiązywania Udziału, na korzystanie z Aplikacji w zakresie niezbędnym do jej utrzymania, rozwoju i hostingu (§ 7, § 11) oraz do prezentacji w portfolio (nazwa Aplikacji, makiety i opis funkcjonalny). Publikacja w portfolio materiałów zawierających dane Partnera lub jego klientów wymaga odrębnej, uprzedniej zgody Partnera.
11. **Komponenty własne Wykonawcy (background).** Wykonawca zachowuje prawa do ogólnych, niezwiązanych z niszą Partnera komponentów, bibliotek, narzędzi i metodyki (dalej „**Komponenty własne**") oraz może wykorzystywać je i wcześniejsze doświadczenia w innych projektach; nie obejmuje to indywidualnych, charakterystycznych elementów Aplikacji Partnera (funkcji rdzeniowej, marki, treści, interfejsu).
    (a) **Wykaz.** Komponenty własne wykorzystane w Aplikacji wskazuje wykaz w Załączniku 4 (sekcja „Komponenty własne Wykonawcy"). Uzupełnienie wykazu komponentów open-source i zasobów osób trzecich (ust. 8 lit. c, Załącznik 4) może nastąpić przy wydaniu Aplikacji i nie zmienia zakresu przeniesienia praw.
    (b) **Licencja dla Partnera.** Do Komponentów własnych wykorzystanych w Aplikacji Wykonawca udziela Partnerowi licencji **niewyłącznej, bezterminowej, nieograniczonej terytorialnie, zbywalnej wraz z Aplikacją i z prawem udzielania sublicencji w tym zakresie**, obejmującej korzystanie, modyfikację i utrzymanie Aplikacji — bez dodatkowych opłat ponad § 5 i § 6.
    ...
    (d) **Zakaz konkurencji.** Wykonawca nie stworzy dla osoby trzeciej aplikacji konkurencyjnej wobec Aplikacji w tej samej niszy przez okres obowiązywania Udziału (symetria wobec § 10 ust. 3).
12. **Autorskie prawa osobiste.** Wykonawca zobowiązuje się nie wykonywać autorskich praw osobistych do rezultatów prac wobec Partnera i jego następców prawnych oraz wyraża zgodę na: rozpowszechnianie Aplikacji i jej elementów anonimowo (bez oznaczania autorstwa), dokonywanie zmian, łączenie z innymi utworami, skracanie i aktualizację. Wykonawca zapewnia, że analogiczne zobowiązania i zgody uzyska od współtwórców i podwykonawców uczestniczących w tworzeniu Aplikacji.
...
1. Po upływie 12 miesięcy od Startu produkcyjnego Partner może w każdej chwili wykupić Udział za cenę równą **36-krotności średniej miesięcznej kwoty Udziału z ostatnich 12 miesięcy**, nie mniej jednak niż **dwukrotność ceny za budowę brutto określonej w § 5 ust. 1** (`{{PROG_WYKUPU_BRUTTO}}` zł brutto). *[DO POTWIERDZENIA — TOMEK: wysokość progu; przyjęto 2× cena budowy brutto]*
2. Z chwilą zapłaty ceny wykupu Udział oraz opieka i serwis z § 7 wygasają, a Wykonawca przekazuje Partnerowi administrację infrastrukturą, komplet dostępów produkcyjnych i pełne repozytorium kodu źródłowego (§ 8 ust. 9 lit. b).

## § 10. Sprzedaż biznesu i kontynuacja

1. Partner może sprzedać Aplikację lub przedsiębiorstwo z nią związane pod warunkiem, że nabywca **przejmie na piśmie obowiązki Partnera z niniejszej umowy** (przejęcie długu za zgodą Wykonawcy, art. 519 k.c.; Wykonawca nie odmówi zgody bez ważnego powodu) — albo że przed zbyciem nastąpi wykup Udziału (§ 9; przed upływem 12 miesięcy od Startu produkcyjnego cena wykupu równa jest 36-krotności średniej miesięcznej kwoty Udziału z dostępnych pełnych miesięcy kalendarzowych od Startu produkcyjnego, nie mniej jednak niż **dwukrotność ceny za budowę brutto określonej w § 5 ust. 1** (`{{PROG_WYKUPU_BRUTTO}}` zł brutto); jeżeli do dnia zbycia nie zakończył się żaden pełny miesiąc kalendarzowy — należna jest wyłącznie ta cena minimalna). *[DO POTWIERDZENIA — TOMEK: wysokość progu; przyjęto 2× cena budowy brutto]*
2. **Zbycie z pominięciem ust. 1.** Jeżeli Partner zbywa Aplikację lub przenosi korzyści z jej eksploatacji na inny podmiot bez spełnienia warunku z ust. 1, cena wykupu obliczona zgodnie z ust. 1 staje się wymagalna; jej zapłata wywołuje skutki wykupu (§ 9 ust. 2).
3. **Kontynuacja.** Do Przychodu wlicza się również wpływy z odpłatnej eksploatacji wersji Aplikacji zmodyfikowanej, rozwiniętej, migrowanej lub rebrandowanej, jeżeli wykorzystuje ona całość albo istotną część kodu, interfejsu, dokumentacji lub opracowań objętych przeniesieniem praw (§ 8); samo rozwiązywanie tego samego problemu lub kierowanie produktu do tej samej grupy odbiorców nie wystarcza. Postanowienie określa jedynie podstawę wynagrodzenia i nie ogranicza działalności Partnera. Kontynuację uwzględnia się wyłącznie do dnia wykupu (§ 9) albo upływu terminu z § 6 ust. 1. Podmiot powiązany oznacza podmiot kontrolowany przez Partnera, kontrolujący go lub pozostający z nim pod wspólną kontrolą, a także osoby najbliższe działające na jego rachunek.

## § 11. Infrastruktura, hosting i koszty zewnętrzne

1. Przez 12 miesięcy od Startu produkcyjnego koszty infrastruktury (hosting, baza danych, wysyłka e-mail, domena) ponosi Wykonawca. Po tym okresie koszty dzielone są: **90% Partner, 10% Wykonawca** — rozliczane na podstawie rzeczywistych faktur dostawców. Po przekazaniu Partnerowi administracji infrastrukturą (§ 8 ust. 9, § 9 ust. 2, § 15) wszystkie dalsze koszty infrastruktury ponosi Partner — udział Wykonawcy w wysokości 10% wygasa (koniec finansowania 10% przez Wykonawcę).
2. **Zmienne koszty zewnętrzne** związane z korzystaniem z Aplikacji przez jej użytkowników i skalą działania — w szczególności prowizje operatora płatności (Stripe), koszty bramek SMS, opłaty za API oraz koszty modeli AI przy wolumenie — obciążają **Partnera jako sprzedawcę (merchant of record)** od pierwszego dnia i nie są objęte ani ceną z § 5, ani Udziałem. Wykonawca nie odpowiada za zobowiązania publicznoprawne i rozliczenia wynikające z przychodów Partnera.
3. **Legalna forma działalności Partnera.** Partner zapewnia, że najpóźniej przed Startem produkcyjnym będzie prowadził działalność w formie umożliwiającej legalną sprzedaż w Aplikacji i rejestrację w systemie płatności (Stripe) jako *merchant of record*.

## § 12. Dane osobowe

1. Administratorem danych osobowych użytkowników Aplikacji jest Partner. Wykonawca oraz dostawcy infrastruktury przetwarzają te dane w imieniu Partnera jako podmioty przetwarzające, na zasadach umowy powierzenia przetwarzania danych (DPA) stanowiącej **Załącznik 2**, zgodnej z art. 28 RODO.
2. Partner jako administrator zapewnia podstawę prawną przetwarzania oraz spełnia obowiązki informacyjne wobec swoich użytkowników. Wykonawca udostępnia niezbędne informacje o dostawcach i lokalizacji przetwarzania (w tym o ewentualnych transferach poza EOG) — szczegóły w Załączniku 2.
...
3. **Żądanie rozpoczęcia świadczenia.** Partner może zażądać rozpoczęcia prac przed upływem terminu do odstąpienia, składając wyraźne oświadczenie (w treści umowy albo w Załączniku 3). Wykonawca rozpoczyna prace dopiero po otrzymaniu takiego żądania (art. 15 ust. 3 oraz art. 21 ust. 2 UoPK).
4. **Skutki odstąpienia przed pełnym wykonaniem.** Jeżeli Partner odstąpi od umowy po zażądaniu rozpoczęcia prac, lecz przed pełnym ich wykonaniem, zapłaci Wykonawcy kwotę proporcjonalną do zakresu świadczeń spełnionych do chwili odstąpienia (art. 35 UoPK). Proporcję ustala się według **Harmonogramu wartości Etapów** (§ 2). Wartości etapów w tabeli są **maksymalne** — płatna jest wyłącznie część faktycznie wykonana w ramach rozpoczętych etapów; etapy nierozpoczęte nie są płatne.
5. **Wygaśnięcie prawa odstąpienia.** Pełne wykonanie samej Usługi Wdrożeniowej nie powoduje utraty prawa odstąpienia od całej Umowy, jeżeli na Wykonawcy spoczywają dalsze świadczenia (w szczególności opieka i serwis z § 7 oraz przeniesienie rezultatów powstałych po odbiorze MVP — § 8 ust. 2). Prawo odstąpienia wygasa wyłącznie w przypadkach wynikających z bezwzględnie obowiązujących przepisów UoPK.
...
1. **Rezygnacja Partnera przed rozpoczęciem prac** — Wykonawca zwraca całość otrzymanego wynagrodzenia (w tym opłatę rezerwacyjną), pomniejszoną wyłącznie o ewentualne koszty rzeczywiście poniesione na wyraźne żądanie Partnera; prawa autorskie nie przechodzą.
2. **Rezygnacja w trakcie budowy** — rozliczenie następuje według Harmonogramu wartości Etapów (§ 2 i § 14 ust. 4): Partner płaci za część faktycznie wykonaną, resztę Wykonawca zwraca. Wobec Partnera będącego Przedsiębiorcą strony mogą to samo rozliczenie stosować odpowiednio, bez odwołania do przepisów konsumenckich.
3. **Po wdrożeniu, a przed osiągnięciem 50 klientów** — jeżeli cena za budowę została zapłacona w całości, prawa autorskie są już przeniesione (§ 8 ust. 2) i pozostają przy Partnerze. Partner może wypowiedzieć opiekę i rozruch prowadzone przez Wykonawcę i samodzielnie prowadzić Aplikację. **Udział — jako zmienna część ceny za przeniesienie autorskich praw majątkowych (§ 6 ust. 1) — trwa do wykupu (§ 9), także po takim wypowiedzeniu** (§ 6 ust. 2). Rezygnacja z dalszej pomocy Wykonawcy nie kończy Udziału; kończy go dopiero wykup.
4. **Cesja na następcę prawnego.** Wykonawca może przenieść prawa i obowiązki z umowy na następcę prawnego (np. w razie przekształcenia formy prowadzenia działalności) za uprzednim powiadomieniem Partnera, bez pogorszenia jego sytuacji.
...
2. **Ostrzeżenie o formie.** Skan lub zdjęcie podpisu ani podpis zaufany (Profil Zaufany) nie zachowują formy pisemnej i nie przenoszą autorskich praw majątkowych, które wymagają tej formy pod rygorem nieważności (art. 53 pr. aut.).
3. **Klauzula ratunkowa (konwersja).** Do czasu zachowania wymaganej formy strony są związane warunkami umowy ze skutkami licencji niewyłącznej na korzystanie z Aplikacji (art. 65 pr. aut.), a przeniesienie praw następuje dopiero po zachowaniu formy pisemnej albo QES i zapłacie ceny.
4. **Przejrzystość warunków cenowych.** Warunki cenowe (§ 5, § 6, § 9, § 10) są jednakowe dla wszystkich partnerów Wykonawcy i zostały przedstawione Partnerowi przed zawarciem Umowy w sposób umożliwiający zrozumienie ich konsekwencji ekonomicznych, wraz z przykładem liczbowym stanowiącym Załącznik 1a.
...
Wkleja się tu zaakceptowany dokument „Zakres MVP" projektu {{NAZWA_APLIKACJI_ROBOCZA}}: funkcje pierwszej wersji, świadome wyłączenia, główny problem i grupa odbiorców (baza dla § 10 ust. 3), granica „poprawka vs nowa funkcja", lista ekranów i funkcja rdzeniowa. Warstwa platformy (stała w każdej budowie): logowanie i konta, panel klienta i panel admina, główna funkcja + 1–2 wspierające, płatności i subskrypcje przez Stripe, strona zapisu/sprzedaży pod markę, automatyzacje i statystyki, regulamin/polityka prywatności/RODO, uruchomienie na żywo + hosting na start.
...
- Przy Przychodzie Aplikacji **10 000 zł miesięcznie** Udział Wykonawcy wynosi **1 000 zł miesięcznie** (10% × 10 000 zł).
- Cena wykupu po upływie 12 miesięcy, przy średniej miesięcznej kwocie Udziału **1 000 zł**, wynosi **36 000 zł** (36 × 1 000 zł — § 9 ust. 1); jest ona wyższa od progu minimalnego (dwukrotność ceny za budowę brutto — § 9 ust. 1), więc próg nie ma tu znaczenia.
- Gdy iloczyn 36 × średnia miesięczna kwota Udziału jest niższy od progu minimalnego (dwukrotność ceny za budowę brutto — § 9 ust. 1), cena wykupu równa jest temu progowi minimalnemu.
- Przy wcześniejszym wykupie w związku ze zbyciem biznesu (§ 10 ust. 1) średnią miesięczną kwotę Udziału liczy się z dostępnych pełnych miesięcy kalendarzowych; jeżeli iloczyn 36 × ta średnia jest niższy od progu minimalnego (dwukrotność ceny za budowę brutto), cena wykupu również równa jest progowi minimalnemu.

Przy cenie za budowę {{CENA_BRUTTO}} zł brutto próg minimalny wynosi {{PROG_WYKUPU_BRUTTO}} zł brutto. Faktyczne kwoty zależą od rzeczywistego Przychodu i stawki {{FEE_PERCENT}}% wpisanej w § 6.

---

# Załącznik 2 — Umowa powierzenia przetwarzania danych osobowych (DPA)

Zawarta jako integralna część umowy głównej, pomiędzy **Partnerem** (Administrator) a **Wykonawcą** (Podmiot przetwarzający). Realizuje wymóg art. 28 ust. 3 RODO.
...
### Rewizja 5 (22.07) — rewizja sprzedażowa (decyzja Tomka)

Umowa jest także narzędziem sprzedaży: usunięto/złagodzono zapisy, które straszą lub rodzą zbędne pytania, o ile ustawa lub inne postanowienia dają ten sam skutek domyślnie. **Ochrona Wykonawcy i zgodność z prawem nietknięte** — cięcia są świadomym „przemilczeniem", nie rezygnacją z ochrony. Poniżej każde cięcie z uzasadnieniem, dlaczego przemilczenie jest bezpieczne (żeby prawnik/Tomek mógł je świadomie przywrócić).

**Cięcia (przemilczenie — skutek zapewnia ustawa albo inny zapis):**
1. **§ 6 ust. 8 (cesja wierzytelności Udziału) — USUNIĘTY w całości** (wraz ze zdaniem o „oznaczalności wierzytelności przyszłych"). *Bezpieczne, bo:* zbywalność wierzytelności wynika z **art. 509 k.c. domyślnie** — zgoda Partnera nie jest potrzebna do skutecznej cesji, a klauzula tylko podsuwała lęk „sprzeda mój dług?". Skutek (Wykonawca może scedować Udział) pozostaje z mocy prawa. Renumeracja: § 6 ust. 9 → **ust. 8**; poprawione odesłanie w § 2 (definicja „Przychód").
2. **§ 15 ust. 4 (śmierć / trwała niezdolność Wykonawcy) — USUNIĘTY w całości.** *Bezpieczne, bo:* Udział jest **wierzytelnością i wchodzi do spadku z mocy prawa** (art. 922 k.c.) — skutek „dziedziczenia Udziału", który Tomek świadomie wybrał w Rewizji 2, realizuje się TAK SAMO, tylko milcząco (bez alarmującego paragrafu „o śmierci" w umowie sprzedażowej). Scenariusz przekazania repozytorium/dostępów po zakończeniu opieki „z jakiejkolwiek przyczyny" (co obejmuje śmierć) jest już pokryty przez **§ 8 ust. 9 lit. c**. Renumeracja: § 15 ust. 5 → **ust. 4**; poprawione odesłanie w § 7 ust. 1 („§ 15 ust. 3–4" → „§ 15 ust. 3"). § 8 ust. 9 lit. c odsyła do § 15 ust. 3 (bez zmian).
3. **§ 10 ust. 2–3 (kara umowna za zbycie z pominięciem procedury) — ZASTĄPIONE jednym neutralnym ust. 2.** Nowa norma (jedna dla wszystkich): zbycie/przeniesienie korzyści bez spełnienia warunku z ust. 1 → **cena wykupu staje się wymagalna**, a jej zapłata wywołuje skutki wykupu. *Bezpieczne, bo:* to **ten sam pieniądz** (= cena wykupu) co dawna „kara umowna wobec Przedsiębiorcy", ale skonstruowany jako **wymagalność ceny za już wykonane przeniesienie praw** — działa wobec WSZYSTKICH (bez rozróżnienia Przedsiębiorca/Konsument/PNPK), brzmi neutralnie i **nie podlega miarkowaniu z art. 484 § 2 k.c.** (to nie sankcja). Usunięto słowa „kara umowna" i odesłanie do art. 484 § 2. Renumeracja: § 10 ust. 4 (Kontynuacja) → **ust. 3**.
4. **§ 13 ust. 6 („Bez kar wobec Konsumenta/PNPK") — USUNIĘTY.** *Bezpieczne, bo:* po zmianie § 10 umowa **nie zawiera już ŻADNEJ kary umownej** — wzmianka „nie zastrzegamy kar" tylko podsuwałaby temat kar, którego w umowie nie ma. Renumeracja: § 13 ust. 7 (Ograniczenie odpowiedzialności) → **ust. 6**.
5. **§ 8 ust. 11 lit. a — UPROSZCZONE:** usunięto zdania o ustalaniu wykazu Komponentów własnych „najpóźniej przed wykorzystaniem" oraz o aneksie pisemnym/QES przed wydaniem. *Bezpieczne, bo:* Partnera chroni **reguła domyślna z lit. c** (element niewpisany do wykazu = objęty przeniesieniem praw) — formalizm aneksowy rodził pytania, a nie dawał Partnerowi realnej dodatkowej ochrony. Zostaje: wykaz w Załączniku 4; uzupełnienie wykazu OSS możliwe przy wydaniu Aplikacji.
6. **§ 8 ust. 8 lit. c — SKRÓCONE:** usunięto ostatnie zdanie („Pominięcie zasobu… stanowi naruszenie obowiązku ujawnienia…"). *Bezpieczne, bo:* odpowiedzialność Wykonawcy wynika i tak z oświadczeń z lit. a (całość praw, brak naruszeń) oraz zasad ogólnych — zdanie było prawniczą nadbudową.

**Przeredagowania (zostaje merytorycznie, bez straszenia):**
7. **§ 6 ust. 1** — „15 lat" ujęte jako **gwarancja końca** („Udział jest ograniczony w czasie: wygasa najpóźniej z upływem 15 lat… a wcześniej — z chwilą wykupu"), nie jako „długość wyroku". Merytoryka (należny do wykupu, max 15 lat od Startu) bez zmian — obrona przed art. 365¹ k.c. utrzymana.
8. **§ 6 ust. 4** — skrócone ~o połowę; zostaje obowiązek zintegrowanego kanału, zakaz omijania systemu, szacowanie Przychodu z możliwością wykazania rzeczywistych kwot i wezwanie 14 dni. Usunięto: wypowiedzenie opieki ze skutkiem natychmiastowym wobec Przedsiębiorcy (eskalacja strasząca) i zdanie „Naruszenie nie wpływa na dalszy obowiązek zapłaty Udziału" (oczywistość, wynika z § 6 ust. 2).
9. **§ 7 ust. 6 (AI)** — złagodzone brzmienie; nazwa „AI Act / rozporządzenie o sztucznej inteligencji" **usunięta z treści operatywnej** (obowiązki regulacyjne „spoczywające na operatorze" wykonuje Partner po przejęciu sterów; Wykonawca podaje modele i ich dostawców).
10. **§ 8 ust. 9** — kolejność pozytyw-najpierw: nowa lit. a = prawo Partnera do kopii repozytorium po zapłacie (dawna lit. b), lit. b = administrowanie/pełne repo przy wykupie (dawna lit. a), lit. c bez zmian. Poprawione odesłanie w § 9 ust. 2 („§ 8 ust. 9 lit. a" → „lit. b"). Treść merytoryczna bez zmian.
11. **§ 1 ust. 3** — skrócone; usunięto zdanie „Ryzyko gospodarcze… obciąża Partnera" (odpychające, wynika z ust. 2 i natury umowy).
12. **§ 1 ust. 5, § 10 ust. 3 (Kontynuacja), § 11 ust. 3, § 16 ust. 2–3** — kompresja redakcyjna bez utraty treści. § 11 ust. 3 do jednego zdania (usunięte zdanie o wstrzymaniu Startu jest pokryte przez § 4 ust. 1 i § 1 ust. 5).
13. **§ 6 ust. 8 (dawny ust. 9, zasady liczenia Przychodu)** — kompresja liter (a)–(g) bez utraty żadnej reguły.

**Cross-doc:** przeszukano folder `prawne-aplikacja/` — żaden inny dokument (Umowa wykupu, Protokół, Checklist, Oświadczenie, Regulamin, NDA) NIE odsyła do renumerowanych jednostek Umowy Budowy (§ 15 ust. 4/5, § 10 ust. 2/3/4, § 13 ust. 6/7, § 6 ust. 9). Odesłania w tych plikach dotyczą jednostek niezmienionych (§ 6 ust. 2/4/5, § 9, § 10 jako całość, § 13 jako całość, § 14, § 15 ust. 3, § 16) albo własnej numeracji tych dokumentów. **Uwaga dla prawnika:** CHECKLIST-PROCES-ZAWARCIA.md wiersz „kary tylko wobec Przedsiębiorcy… wobec Konsumenta/PNPK — wymagalność ceny (§ 10)" opisuje STARĄ mechanikę § 10 (nie jest odesłaniem numerycznym, więc nietknięte) — przy aktualizacji checklisty ujednolicić z nowym § 10 (jedna norma: wymagalność ceny wykupu dla wszystkich, bez kar).

### Rewizja 4 (22.07): ceny = placeholdery (decyzja Tomka — cena wg faktycznej oferty zakupu; standard obecnie 12 500 netto / 15 375 brutto / rezerwacja 500 / po zaliczeniu 14 875 / próg 2× brutto = 30 750 przy standardzie)

Cena za budowę jest **INDYWIDUALNA** — usługa może zostać sprzedana taniej lub drożej niż standard, a umowa odzwierciedla kwotę faktycznie zapłaconą. W treści OPERATYWNEJ ŻADNA kwota ceny nie jest zaszyta na sztywno — wyłącznie placeholdery `{{...}}`:
- **§ 5 ust. 1 i box podsumowania** — `{{CENA_NETTO}}`, `{{CENA_BRUTTO}}`, `{{REZERWACJA_KWOTA}}`, `{{KWOTA_POZOSTALA_BRUTTO}}`;
- **§ 9 ust. 1 i § 10 ust. 1 (próg wykupu)** — formuła „dwukrotność ceny za budowę brutto określonej w § 5 ust. 1" + `{{PROG_WYKUPU_BRUTTO}}`;
- **Załącznik 1a** — przykłady Udziału pozostają liczbowe (10 000 → 1 000; wykup 36 × 1 000 = 36 000), wzmianki o progu = formuła, a wartość progu przy cenie standardowej dopisano jednym zdaniem z `{{CENA_BRUTTO}}` / `{{PROG_WYKUPU_BRUTTO}}`.

Wartości standardowe (12 500 / 15 375 / 500 / 14 875 / 30 750) występują od tej rewizji **wyłącznie w NOTATKACH** jako punkt odniesienia — nigdy w renderowanej treści.
...
11. **§ 9 ust. 1 i § 10 ust. 1** — próg minimalny wykupu = **dwukrotność ceny za budowę brutto** (w treści operatywnej formuła + `{{PROG_WYKUPU_BRUTTO}}`; przy cenie standardowej 30 750 zł brutto — Rewizja 4), OBUSTRONNY: dotyczy zarówno standardowego wykupu po 12 mies. (§ 9), jak i wcześniejszego przy zbyciu (§ 10; 36× śr. z pełnych miesięcy, nie mniej niż próg; zero pełnych miesięcy = tylko próg). Asymetria z Rewizji 2 (próg tylko w § 10) USUNIĘTA decyzją Tomka 22.07. Wykup jest UPRAWNIENIEM Partnera (opcja, nie obowiązek), co ogranicza ryzyko abuzywności progu wobec Konsumenta/PNPK.
12. **Box na górze** — dodano „nie dłużej niż przez 12 miesięcy" do opisu rozruchu.
13. **§ 5 ust. 1 + box (poprawka nr 14, agent spójności)** — konwencja BRUTTO: przy cenie standardowej po zaliczeniu rezerwacji 500 zł pozostaje **14 875 zł brutto** (koniec z rozbiciem na „12 000 zł netto"); od Rewizji 4 kwoty te są placeholderami (`{{CENA_NETTO}}`, `{{CENA_BRUTTO}}`, `{{REZERWACJA_KWOTA}}`, `{{KWOTA_POZOSTALA_BRUTTO}}`); flaga do księgowej o momencie VAT zaliczki i charakterze kaucji zwrotnej.
...
2. **[ROZSTRZYGNIĘTE — TOMEK 22.07: wariant z DZIEDZICZENIEM ZATWIERDZONY; marker rekomendacyjny usunięty z § 15 ust. 4.] Śmierć / trwała niezdolność Tomka (§ 15 ust. 4).** Po finalnej opinii gpt-5.6-sol (22.07) ODWRÓCONO na wariant z DZIEDZICZENIEM: Udział jako odroczona część ceny za przeniesienie praw NIE wygasa ze śmiercią — przysługuje następcom prawnym Tomka do wykupu (§ 9) albo upływu terminu z § 6 ust. 1; jeżeli w 30 dni nikt zdolny nie przejmie opieki, opieka i serwis wygasają, a następcy przekazują Partnerowi repozytorium, administrację infrastrukturą i dostępy (bez gaszenia Udziału). Obie opinie prawne wskazały, że wariant „Udział wygasa ze śmiercią" podważa konstrukcję Udziału jako ceny za prawa i pozbawia rodzinę przychodu. **Rewizja 5 (22.07): § 15 ust. 4 (śmierć / trwała niezdolność) PRZEMILCZANY — usunięty z treści umowy jako alarmujący w dokumencie sprzedażowym. Skutek dziedziczenia Udziału zapewnia ustawa (Udział = wierzytelność wchodząca do spadku, art. 922 k.c.), a przekazanie repozytorium/dostępów po zakończeniu opieki „z jakiejkolwiek przyczyny" (co obejmuje śmierć) pokrywa § 8 ust. 9 lit. c. Decyzja Tomka o dziedziczeniu pozostaje więc zrealizowana, tylko milcząco. Aby wpisać wariant wprost (dziedziczenie albo wygaśnięcie jako gest wobec klientów), przywróć ustęp w brzmieniu z Rewizji 2.**
...
4. **VAT (§ 5 ust. 2, § 6 ust. 5).** Status VAT stron, stawka oraz moment obowiązku podatkowego przy automatycznym poborze `application_fee` (art. 19a ust. 3 i 8 ustawy o VAT) — do ustalenia z księgową; rozważyć interpretację indywidualną KIS. Przy standardowej cenie kwota brutto 15 375 zł zakłada 23% VAT; przy cenie indywidualnej brutto = netto × 1,23 (stawka obowiązująca w dniu wystawienia faktury).
5. **Placeholdery do wypełnienia przy generowaniu:** `{{FEE_PERCENT}}` (default 10), `{{TERMIN_TYGODNI}}` (zwykle 4–8 tyg.), `{{NAZWA_APLIKACJI_ROBOCZA}}`, dane stron. **UWAGA:** placeholder `{{BUDZET_ROZRUCHU}}` z wcześniejszego draftu został USUNIĘTY — § 1 ust. 4 rozstrzyga wprost, że budżet reklamowy rozruchu finansuje Partner (pkt 9), więc dodatkowe pole w `contract_fields` nie jest potrzebne. **NOWE pola cenowe (Rewizja 4)** wymagane w systemie renderującym (`contract_fields` + mapa w `tn-app/projekt.html` oraz `wfa-portal`): `{{CENA_NETTO}}`, `{{CENA_BRUTTO}}`, `{{REZERWACJA_KWOTA}}`, `{{KWOTA_POZOSTALA_BRUTTO}}`, `{{PROG_WYKUPU_BRUTTO}}`. Analogia do pułapki PESEL (pkt 3): niepodstawiony placeholder wycieka na wydruk, więc każde z tych pól MUSI być obsłużone w mapie renderującej. Wartości liczbowe najlepiej wyliczać automatycznie z ceny z faktycznej oferty zakupu: `CENA_BRUTTO` = `CENA_NETTO` × 1,23; `KWOTA_POZOSTALA_BRUTTO` = `CENA_BRUTTO` − `REZERWACJA_KWOTA`; `PROG_WYKUPU_BRUTTO` = 2 × `CENA_BRUTTO`.
...
- **~~Miarkowanie kary umownej z § 10 ust. 2 (art. 484 § 2 k.c.)~~ — NIEAKTUALNE od Rewizji 5.** Kara umowna z § 10 USUNIĘTA; zastąpiona konstrukcją wymagalności ceny wykupu (jedna norma dla wszystkich). Skoro to zapłata ceny za już wykonane przeniesienie praw, a nie sankcja, **nie podlega miarkowaniu z art. 484 § 2 k.c.** — do potwierdzenia, czy sąd nie zakwalifikuje jej jako ukrytej kary wobec Konsumenta/PNPK (ocena pod kątem art. 385¹ k.c., nie 484 § 2).
- **Kontrola abuzywności** § 6, § 9, § 10 wobec PNPK (art. 385¹ w zw. z 385⁵ k.c.). **Usunięto klauzulę o „indywidualnym uzgodnieniu" (§ 16 ust. 4)** jako dowodowo szkodliwą i sprzeczną z „Udział nie podlega negocjacji" — zastąpiono ją klauzulą przejrzystości warunków cenowych + przykładem liczbowym (Załącznik 1a). ~~Zawężenie kar do Przedsiębiorców (§ 10 ust. 3, § 13 ust. 6)~~ — od Rewizji 5 umowa **nie zawiera żadnych kar umownych** (§ 10 = wymagalność ceny wykupu dla wszystkich; usunięto też § 13 ust. 6 „bez kar"), co samo w sobie zmniejsza pole kontroli abuzywności; ocenić, czy wymagalność pełnej ceny wykupu wobec Konsumenta/PNPK nie jest nadmierna (wykup jest jednak uprawnieniem, nie obowiązkiem — argument obronny).
...
- **~~Cesja wierzytelności przyszłych z Udziału (§ 6 ust. 8)~~ — NIEAKTUALNE od Rewizji 5.** Klauzula cesji USUNIĘTA (przemilczenie) — zbywalność wierzytelności Udziału wynika z art. 509 k.c. domyślnie. Gdyby prawnik chciał ją przywrócić dla pewności co do oznaczalności wierzytelności przyszłych, wróć do dawnego § 6 ust. 8.
