...
- **NDA W LEJKU (decyzja Tomka):** w rozmowie tylko WSTĘPNY zarys; szczegóły = etap po rezerwacji i tam podpisywane NDA. Wzór `NDA-JEDNOSTRONNE-DRAFT.md` po recenzji prawnej (Rewizja 4: pozytywny zakaz „nie zbuduję produktu na Twoim pomyśle", załączniki w definicji, pierwszeństwo Umowy Budowy) opublikowany jako strona `tomekniedzwiecki.pl/aplikacja/nda/` (baner WZÓR POGLĄDOWY, bez auto-zawarcia przez ujawnienie i bez NOTATEK). Prompt: obiekcja kradzieży pomysłu prowadzi do NDA + linku; wzajemne NDA zostaje wewnętrzne (etap współpracy).
