...
- **⛔ MODEL UŻYCIA (twarda bramka — feedback Tomka 22.07: „fabryka MUSI ROZUMIEĆ produkt: jak się go używa, jak działa; takie błędy nie mogą się powtarzać"):** obowiązkowy blok `KARTA.product.model_uzycia`. Z FAKTÓW aukcji (`ali_snapshot`: specs/opis/warianty + klipy referencyjne) — NIE zgadując — operator spisuje: (a) **`interakcja`** = co użytkownik FIZYCZNIE robi z ELEMENTEM STEROWANIA i jak go aktywuje (nazwij element: przycisk X/uchwyt/spust/głowica; sposób: „kciuk TEJ SAMEJ dłoni dociska przycisk — wyraźny docisk; przewinięcie ekranu = SKUTEK kliku"); (b) **`nie_robi`** = ZAKAZY SCEN, interakcje których być NIE MOŻE (gest palcem w powietrzu w kierunku ekranu, palec wskazujący celujący w telefon, dotykanie ekranu, przechylanie/obrót jak żyroskop/joystick, trzymanie odwrotnie, użycie jak inny produkt) — wchodzą też do negative promptu; (c) **`noszenie`** = jak produkt jest noszony/trzymany (na którym palcu / oburącz za uchwyty / hands-free na lędźwiach). **KAŻDA klatka-klucz (KROK 5) i KAŻDY prompt sceny MUSI być zgodny z `model_uzycia`** — do promptu wchodzi opis INTERAKCJI, do negative zakazy z `nie_robi`. Wierność PRODUKTU ≠ wierność UŻYCIA (incydent Skrolik 22.07: pierścień-pilot pokazany jak sterowanie gestem/żyroskopem zamiast KLIKANIA przycisków kciukiem; Ugniatek: głowice od ciała). qa_gate/product_gate dostają pytanie „czy scena pokazuje POPRAWNE użycie wg `model_uzycia`?" → niezgodność = **FAIL klipu** (KROK 7 pkt 3d). SSOT: `GENERATOR-VIDEO-STRATEGIA.md` 0k.
...
3. Przegląd agentem z checklistą KARTY: **WIERSZ NA KLATKĘ** (nr, #obiektów produktu, #dłoni, flagi) — ZAKAZ oceny zbiorczej (zbiorcza przepuściła 2× duplikat 17.07). Sprawdzasz: liczbę egzemplarzy, `functional_count` (ciągłość), dłonie, twarz+oczy vs face_ref, tło, fizykę w ruchu, glify, afordancję, **ANATOMIĘ POSTACI (pkt 3b)**.
   ...
   - **3d. MODEL UŻYCIA (feedback Tomka 22.07 — Skrolik/Ugniatek; osobna oś od 3b anatomii i od wierności kształtu):** per klip odpowiedz „czy scena pokazuje POPRAWNE użycie wg `KARTA.product.model_uzycia`?" — właściwy ELEMENT STEROWANIA jest aktywowany tak jak w realu (Skrolik: kciuk DOCISKA przycisk, palec wskazujący NIE jest wyciągnięty i NIE celuje w ekran; masażer: głowice DO ciała, chwyt oburącz za uchwyty), i ZERO interakcji z `model_uzycia.nie_robi` (gest w powietrzu w kierunku ekranu, dotyk ekranu, obrót jak żyroskop, użycie jak inny produkt). Niezgodność = **REJECT klipu nawet gdy produkt idealnie wierny** — reklama ucząca błędnej obsługi odstrasza (produkt „nie działa" w oczach leada).
...
- [ ] **Użycie POPRAWNE wg `model_uzycia`** (KROK 7 pkt 3d): właściwy element sterowania aktywowany jak w realu (przycisk klikany, głowice do ciała itd.); ZERO interakcji z `model_uzycia.nie_robi` (gest w powietrzu w kierunku ekranu, dotyk ekranu, obrót jak żyroskop, użycie jak inny produkt).
