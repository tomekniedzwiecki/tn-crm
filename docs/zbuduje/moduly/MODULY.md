...
| **sticky-buy** | @1 | `sticky-buy@1.html` | loczek@8726382b · 2026-07-17 | skórka: tokeny/kolory/radius, treść (marka/cena/metody), href. NIETYKALNE: fixed + translateY(120%)→.show, `@media(min-width:900px){display:none}`, body padding-bottom, IO na `.hero` + IO na `#zamow` (poprawka 22.07: .show TYLKO poza hero I poza checkoutem — twardy wymóg checkout-inline@2, sticky przysłaniał CTA kasy; wdrożone: skrolik/odsaczek/ugniatek). ⛔ pokazywanie od razu / na desktopie / bez obserwera #zamow przy checkout-inline. |
...
| **footer** | @1 | `footer@1.html` | drapek@2026-07-18 · montaż finalny | skórka: tokeny/kolory/radius, treść (marka/claim/rating/copyright/teksty linków), favicon. NIETYKALNE: 3 strefy `.foot-top` (marka+claim+rating / linki prawne / zaufanie) + `.foot-bottom`, KOMPLET linków (Regulamin·Polityka·Cookies·Zwroty·Dostawa·Kontakt) z placeholderami `{{*_URL}}` (strony publikuje legal-forge, krok pl_prawne), pay-badges kanoniczne, siatka 3→2→1 (bp 820/520), touch ≥40px, ZERO JS. ⛔ „stopka = jedna linijka wyśrodkowana" / wordmark z obrazka. |
...
| **checkout-inline** | **@3** | `checkout-inline@3.html` (**KANON 23.07** — paczkomaty z mapką: broker-config → BliskaPaczka/InPost Geowidget, pola `pickupPoint*`, adres opcjonalny przy paczkomacie; wpięcie `inject-checkout3.py`; @2/@1 ZAMROŻONE, zostają w repo) | zbudowany od zera · 2026-07-20; @2 · 2026-07-21 (CHECKOUT-INLINE-SPEC + research Baymard/PL-prawo); @3 · 2026-07-23 (mapka paczkomatów, decyzja Tomka) | skórka: tokeny `--zc-*` (paleta/promienie/cień/font), copy nagłówka (`.zc-eyebrow`/`.zc-h2`), chipy trust + dane firmy `.zc-merchant`, miniatura/nazwa/cena z KONFIGURACJI, opcjonalny dekoracyjny backdrop, WARIANT PRODUKT-OBOK (karta produktu obok modułu → `.zc-summary` hidden, total = `.zc-total`; LL-075). NIETYKALNE: CAŁY JS (config z wf2-landing-api blok `platform` LUB `data-zc-config`; clientId `trv_cid`→localStorage→UUIDv7; lazy-load dostaw/płatności; flow cart/item→checkout-details→order/cart; BLIK 6-box + polling 2,5 s/max 90 s; COD inline; redirect online; walidacja PL on-blur), STRUKTURA i KOLEJNOŚĆ pól (Imię i nazwisko→Telefon→E-mail→Ulica i numer→Kod→Miejscowość, 1 kolumna, etykiety nad polami, autocomplete+inputmode+enterkeyhint), brzmienie przycisku **„Zamawiam i płacę"** / przy wybranym COD **„Zamawiam i płacę przy odbiorze"** (dynamiczne `ctaLabelText()`; oba komunikują obowiązek zapłaty — art. 17 ust. 3–4 UoPK; standard fabryki, decyzja Tomka 21.07), **CTA BEZ KWOTY (@2)**, **PŁATNOŚĆ BEZ PRESELEKCJI (@2)** — żadna metoda domyślnie nie zaznaczona, COD-copy (microcopy+label) WYŁĄCZNIE po świadomym wyborze, submit bez wyboru = „Wybierz metodę płatności." (slot `data-err="payment"`), microcopy pod CTA startuje hidden, checkbox regulaminu NIE pre-checked, eventy (trevio addToCart/beginCheckout/addShippingInfo/paymentInitiated/purchase\|purchaseOnDelivery; Meta AddToCart/InitiateCheckout@submit + Purchase eventID=orderId TYLKO COD/BLIK-sukces). Desktop ≥900px: formularz + sticky kolumna podsumowania. **Layout: STEPS = JEDYNY (@2, decyzja Tomka 21.07 — root zawsze `data-zc-layout="steps"`, `initSteps()` bezwarunkowo; markup flat = wyłącznie degradacja bez JS; A/B flat-vs-steps ODWOŁANE)** (steps = micro-konfigurator: 4 wizualne kroki Kontakt[E-mail→Telefon]/Adres/Dostawa/Płatność na JEDNEJ stronie, bez „Dalej"; reorder pól przez DOM przy init, NIE CSS `order`; auto-advance on-blur + zwijanie ukończonych do podsumowania + edycja wstecz = NIETYKALNE; skórka = tokeny `--zc-step-*` + tytuły kroków); włączany `data-zc-config.layout='steps'` LUB atrybut `data-zc-layout="steps"`. **identify** (oba layouty): `window.trevio.identify({EmailHash\|PhoneHash})` po pozytywnej walidacji e-maila/telefonu on-blur, raz per wartość. ⛔ pole kodu rabatowego widoczne / liczniki presji / rejestracja / 2 kolumny pól na mobile / pre-checked checkbox / autocomplete="off" / label „Zamawiam"/„Kup teraz" / fbq Purchase przy redirect / ukrywanie COD. |
...
- **2026-07-22 — checkout-inline@2: CTA MOBILE → FORMULARZ (LL-052, feedback Tomka: „CTA
  ...
  brak `.zc-form` LUB form ukryty — guard `!form || !form.offsetParent` (korekta 22.07: w preview .zc-form bywa display:none → martwe kliknięcia) = nawigacja domyślna; desktop bez zmian (góra sekcji — karta+formularz
...
- **2026-07-21 — checkout-inline@2 (NOWA WERSJA — porządek w checkoucie, decyzje Tomka):**
  ...
  „Wybierz metodę płatności." (nowy slot `data-err="payment"`, focusField 'payment');
  microcopy pod CTA startuje puste/hidden (koniec ze stałym „Płacisz dopiero przy odbiorze.").
  (2) **CTA BEZ KWOTY** — `.zc-cta-amt` usunięte (markup+CSS+JS); kwota tylko w podsumowaniu.
  (3) **STEPS = JEDYNY LAYOUT** — rozstrzygnięcie flat-vs-steps decyzją (nie A/B); root
  zawsze `data-zc-layout="steps"`, `initSteps()` bezwarunkowo; flat = degradacja bez JS.
  Kopie 1:1: mata, usmieszek-checkout. @1 zostaje w repo (historia).
