...
| `ads_budzet` | Budżet | client; limit = fabryka | klient zasila SWOJE konto — **prepaid / płatności ręczne** (BLIK/przelew/PayU; decyzja Tomka 22.07), karta = wyjątek (wtedy główna + zapasowa); doładowanie ZAWSZE z Ustawień płatności KONKRETNEGO konta (L1: ogólny przelew utyka na saldzie profilu). Budżet 1000 zł = 500 test / 500 skala. **Limit wydatków konta (bezpiecznik) ustawia FABRYKA** przez API po `WF2_META_TOKEN` — nie klient |
