...
- **F2 makiety 🏁** → `lp_makiety` · fields {sekcje_count, makiety, tor_i, akcept} · REHOST każdej makiety do `bud-assets/<slug>/makiety/` (`storage_upload(..., max_width=1440, to_webp=True, quality=82)`) → `artifact_add` kind `makieta` (mobile → `makieta_mobile`), `meta={section:'03-problem', viewport:'desktop'|'mobile'}`.
  > 🚫 **MAKIETY OBOWIĄZKOWE — ZAKAZ SUBSTYTUTU (LL-078).** Makiety CAŁEJ STRONY (sekcja po sekcji, desktop+mobile, `makiety/NN-sekcja.png`) generuje się **torem LOKALNYM gpt-image** — wzorzec `FABRYKA-*/makiety/_gen.py` (`/v1/images/generations` + `/edits`, klucz `OPENAI_API_KEY` z `.env`; tak zrobił się Zaklipek=22 i ssawek=28). **NIGDY** sceny/fal jako makiety, **NIGDY** puste makiety. Tor EDGE `genimg.py` wymaga `WF2_GEN_SECRET`; jego brak (403) **NIE** jest wymówką do pominięcia makiet — **fallback = tor lokalny, a jeśli i on padnie → HARD STOP i zgłoś Tomkowi**, nie buduj dalej bez makiet. Publish jest twardo bramkowany: `platform-sync _makiety_gate` (<6 makiet strony ∧ <12 wierszy SSIM = SystemExit), więc landing bez makiet **fizycznie nie wejdzie live**. ZAKAZ traktowania makieta-diff FAIL w gate-check jako „false positive do zignorowania".
...
1. **ETAP DOPASOWANIA — OSOBNA FAZA, DO WYCZERPANIA (wzmocnione przez Tomka 16.07: „strona
   ...
   **📐 SKALA TYPOGRAFII = JAWNY PUNKT RUBRYKI · SSIM<0.6 = TWARDY FAIL DOWODU · PEŁNY ZRZUT captureBeyondViewport@900 = OBOWIĄZKOWY (Skrolik LIVE 22.07 → LL-054; Tomek: „landing wygląda totalnie inaczej niż makiety, jakby projekt się nie dokończył" — WSZYSTKIE nagłówki były ~2× za małe, a rubryka wpisała 5×T tłumacząc niski SSIM „artefaktem capture").**
   - **SSIM sekcji < 0.60 = TWARDY FAIL dowodu — NIE do przegadania frazą o artefakcie/capture/„AI-makieta nie dyskryminuje".** Niski SSIM ZOBOWIĄZUJE do udowodnienia zgodności SKALI i KOMPOZYCJI (rozmiar/pozycja kolumn, wysokość kadrów, wielkość typografii), nie wolno go zamknąć samą adnotacją. Dopiero gdy skala+kompozycja są udowodnione WIERNE **i** sekcja jest FOTO-DOMINOWANA (hero/scenowe z dużym kadrem foto), niski SSIM jest LEGALNIE ograniczony różną TREŚCIĄ zdjęć (real vs AI-makieta tną się ~0.40–0.56 — zdjęć klienta SIĘ NIE podmienia na AI); wtedy decyduje H1-delta + rubryka skali + wizualna zgodność kompozycji, NIE goła liczba. FOTO-cap NIE jest wymówką dla sekcji KODOWYCH/tekstowych ani dla realnego rozjazdu skali.
   - **Skala typografii vs makieta = OSOBNY, JAWNY punkt rubryki (6. pole „skala_typo").** Tytuł hero na renderze ma mieć wysokość **±20% tytułu makiety PO NORMALIZACJI SZEROKOŚCI** (analogicznie nagłówki H2 sekcji). Metryka gotowa: `sekcja-diff.py` blok **DELTY POMIAROWE** — „H1 render Npx vs makieta Mpx (±%)", gdzie makieta = IR `scale_px_norm.H1`. |ΔH1| > 20% = FAIL punktu skali. Typowa patologia (Skrolik): clamp cap za niski — `--h1-d` cap 34px vs makieta ~58px norm; `--h2-d` cap 30px vs 44–75px norm → fix = podniesienie clampów + per-sekcja override nagłówków „dużych".
   - **Pełnostronicowy zrzut `captureBeyondViewport` przy NORMALNYM viewporcie (width×900 desktop / ×844 mobile) = OBOWIĄZKOWY dowód finalny.** ⛔ ZAKAZ ustawiania `Emulation.setDeviceMetricsOverride height=scrollHeight` — jednostki `vh` liczą się wtedy od gigantycznego viewportu (100vh/78vh EKSPLODUJĄ), pełny zrzut KŁAMIE: mały tytuł „pływa" w morzu pustki, skala typografii NIEWIDOCZNA (wygląda na „rozjechany layout", nie „font za mały"). Kanon: `full-shot.py` / `fullcap` — force-final + scroll dół→góra (lazy-img) + clip 0..docH.
...
5. **WARIANT PRODUKT-OBOK (Tomek 23.07, Rozmrozik: „[podsumowanie produktu] jest w tej
   sekcji po lewej stronie, więc tutaj nie jest potrzebne — cały ten box jest poświęcony
   na dane do zamówienia"):** gdy sekcja `#zamow` ma WŁASNĄ kartę produktu obok/nad
   modułem (packshot+nazwa+cena, układ `.zm-grid`), pasek podsumowania modułu
   (`.zc-summary`: thumb+nazwa+„1 szt."+tabelka Produkt/Dostawa/Pobranie/Razem) =
   DUPLIKAT → **OFF**: atrybut `hidden` na `.zc-summary` + skórkowy
   `.zc-checkout .zc-summary{display:none!important}`. Kwotę łączną przy CTA daje
   istniejący `.zc-total` „Razem do zapłaty" (element modułu, aktualizowany JS — NIE
   dokładać własnego totalu, będzie dublował). Zasada: **jedna informacja pokazana RAZ**;
   box kasy = wyłącznie dane do zamówienia + total + CTA. Landing bez karty produktu
   w sekcji (moduł = całe podsumowanie, np. rozgrzewek/ugniatek) → pasek ZOSTAJE.
   DOM paska NIE usuwać (JS aktualizuje jego pola — usunięcie = ryzyko wyjątku).
