...
awans sub-kroku `step_update(...)`, artefakty oglądalne `artifact_add(...)`, koszt `cost_add(...)`,
wpis osi czasu `activity_add(...)`. Mapa faza→sub-krok `agr_*` = sekcja „EMISJE DO PANELU" niżej.
...
(kind='link', url=manus.im/app/{task_id})`; `cost_add(step_key='ads_grafiki', kind='manus',
note='<slug> 3 kreacje')`.
...
**Narzędzie:** `ad-gate.py` (miniatury @320px, safe-margin, kontrast) + odczyt agentowy tekstu.
...
(zostaw sam headline + „Kup teraz", drop badge). → dowód: `dowody/thumb-<angle>-320.png` +
`dowody/report.json`.
...
**Emisja:** `agr_final` → `done`; **PUBLIC** 3 kreacje → `bud-assets/<slug>/ads/`
(`artifact_add(kind='ad_creative')`); `cost_add` total-korekta; `activity_add('ads_done', '🏁 …')`.
...
`*_916*` i flaguje tekst w strefie centralnej. Gdy fabryka wejdzie w 9:16 — produkować NATYWNIE
...
| G2 (Manus) | `agr_generacja` → done / blocked | note z task_id; `artifact_add(kind='link', url=manus.im/app/{task_id})`; `cost_add(kind='manus', note='<slug> 3 kreacje')` |
...
| G7 (rejestr) | `agr_final` → done | **PUBLIC** 3 kreacje → `bud-assets/<slug>/ads/` (`kind='ad_creative'`, meta={angle,format}); `creative_upsert` (media_type=image, ai_labeled=true); `cost_add` total-korekta; `activity_add('ads_done', '🏁 …')` |
...
**Koszty:** silnik = **Manus ~$0.30/task** (1 task = 3 kreacje → ~$0.10/kreacja). Rejestrowane
w `wf2_costs` (`step_key='ads_grafiki'`, stage=5, kind='manus'), suma w nagłówku bloku panelu.
