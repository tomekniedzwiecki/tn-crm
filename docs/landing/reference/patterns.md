...
## ⚠️ REGUŁA KRYTYCZNA: MAX 3 patterns per landing — TYLKO dla MODE=paradigm

> **Ta reguła obowiązuje WYŁĄCZNIE gdy `_brief.md` zawiera `MODE: paradigm`.** Dla `MODE: template` nie ma limitu — szablon może używać wszystkich 22 signature snippetów (editorial numeral + magazine numbering + bento spotlight + border beam + marquee + …). To jest intended behavior szablonu.
>
> **Memory: `feedback-landing-structural-uniqueness.md`** (2026-04-20). W MODE=paradigm używanie 8-10 patternów z tej biblioteki w JEDNYM landingu = utrata strukturalnej unikalności. Każdy landing paradigm wyglądał jak rodzeństwo poprzednich, bo wszystkie używały tych samych signature elementów.
>
> **Reguła (MODE=paradigm):** MAX **3** snippety sygnaturowe (patterny #1-#10 + #17-#21 — czyli editorial/signature/animation) na landing.
>
> **Bez limitu (obie ścieżki):** safety primitywy #11-#16 (fade-in safe, dual-bank mobile, header solid, trust strip, magnetic CTA, cookie banner, hamburger) — to są bezpieczniki, nie charakter.
>
> **Dlaczego (MODE=paradigm):** jeśli każdy paradigm landing ma oversized numeral + magazine eyebrow + editorial italic, to wszystkie wyglądają jak Paromia — podczas gdy paradigm ma dawać strukturalnie INNY układ.
>
> **Jak wybrać 3 (MODE=paradigm):** w briefa sekcja 8 ustal **JEDEN** główny signature (hero dominant) + max 2 pomocnicze w sekcjach niżej. Reszta — nie używaj, nawet jeśli „pasuje". Mniej > więcej.
>
> **Forbidden combinations (MODE=paradigm — kumulują się w jeden szablon):**
