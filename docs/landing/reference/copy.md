# Copy & Conversion Playbook

> Referencja czytana w trakcie ETAP 2 (generate) i ETAP 4 (design polish). Hero Headline + Senior Copywriter Playbook + Conversion Boosters.

## Kiedy czytać
- **ETAP 2 (generate)** — kiedy piszesz tekst sekcji
- **ETAP 4 (design polish)** — przy dopracowaniu offer boxa i CTA

## Cross-references
- Implementacja HTML offer box → [`04-design.md` sekcja H](../04-design.md)
- Zakazane frazy (24h/COD/raty/PayPo/Klarna) → [`reference/safety.md` reguła #6](safety.md)
- Patterns CTA (magnetic, shimmer button) → [`reference/patterns.md`](patterns.md)

---

# Część 1 — Hero Headline (NAJWAŻNIEJSZY ELEMENT)
...
**⚠️ Zakazy:** patrz [`reference/safety.md` reguła #6](safety.md) — nigdy „24h", „magazyn PL", „COD", „raty", „PayPo", „Klarna", „Twisto".
...
**Pełna implementacja HTML/CSS:** [`04-design.md` sekcja H — Offer Box 15-point checklist](../04-design.md).
...
| **Odbierz + obietnicę** | "Odbierz swój Prasik" |
...
- [ ] Zero zakazanych fraz (24h, magazyn PL, COD, raty, PayPo, Klarna)?
...
```css
.mobile-sticky-cta { display: none; position: fixed; bottom: 0; left: 0; right: 0;
...
.sticky-btn { background: var(--primary); color: white; padding: 12px 24px;
...
```css
.cta-button { min-height: 48px; min-width: 120px; padding: 14px 24px; }
.nav-link { padding: 12px 16px; }
.faq-question { padding: 16px 20px; min-height: 56px; }
```

Pełna mobile procedura → [`06-mobile.md`](../06-mobile.md).
...
.inline-cta:hover { gap: 12px; }
.inline-cta .arrow { transition: transform 0.2s; }
.inline-cta:hover .arrow { transform: translateX(4px); }
```
...
- **Magnetic CTA Button** (kursor przyciąga) → [`reference/patterns.md` #19](patterns.md)
- **Shimmer Button** (premium feel) → [`reference/patterns.md`](patterns.md)
- **Number Counter animation** (hero stats) → [`reference/patterns.md` #18](patterns.md)
- **Offer Box Visual implementation** → [`04-design.md` sekcja H](../04-design.md)
