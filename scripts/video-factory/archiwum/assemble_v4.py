...
PREFIX = os.environ.get("TALK_PREFIX", "oh")
V4 = {"0": "oh2_s0.mp4", "1": "c1_v2.mp4", "2": "oh2_s2.mp4", "8b": "oh2_s8b.mp4", "11": "oh2_s11.mp4"}  # 'oh' = OmniHuman, 'ka' = Kling Avatar
PLAN = [
...
def src(sid):
    if sid in V4 and os.path.exists(os.path.join(G, V4[sid])):
        return os.path.join(G, V4[sid])
    if sid in TALK_SIDS:
...
def main():
    ...
    run(["ffmpeg", "-v", "error", "-i", joined, "-vf",
         ...
         "vignette=PI/5.5,noise=c0s=14:c0f=t+u",
    ...
    final = os.path.join(OUT, "ad_v4.mp4")
    fc = (
    ...
    run(["ffmpeg", "-v", "error", "-i", graded, "-i", vo, "-i", mus,
         ...
         "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-ar", "48000", "-t", f"{total:.3f}", "-y", final])
