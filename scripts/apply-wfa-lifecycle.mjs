#!/usr/bin/env node
// apply-wfa-lifecycle.mjs — aplikuje 20260722s_wfa_lifecycle.sql przez Supabase
...
//
// Dodaje kolumnę wfa_projects.lifecycle (active|paused|cancelled|completed) i
...
//
// Uruchom: node scripts/apply-wfa-lifecycle.mjs
...
const die = (m) => { console.error(`[apply-wfa-lifecycle] BŁĄD: ${m}`); process.exit(1); };
const log = (m) => console.log(`[apply-wfa-lifecycle] ${m}`);
...
(async () => {
  const mig = readFileSync(join(ROOT, 'supabase', 'migrations', '20260722s_wfa_lifecycle.sql'), 'utf8');
  log('Aplikuję 20260722s_wfa_lifecycle.sql …');
  await sql(mig);
  log('  ✓ zastosowana');
  ...
  const col = await sql(
    `SELECT column_name, data_type, column_default, is_nullable
      ...
      WHERE table_schema='public' AND table_name='wfa_projects' AND column_name='lifecycle'`);
  const dist = await sql(
    `SELECT lifecycle, count(*) c FROM public.wfa_projects GROUP BY lifecycle ORDER BY lifecycle`);
  const chk = await sql(
    `SELECT 1 FROM pg_constraint
      WHERE conrelid='public.wfa_projects'::regclass AND contype='c'
