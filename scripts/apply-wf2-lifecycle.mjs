#!/usr/bin/env node
// apply-wf2-lifecycle.mjs — aplikuje 20260721_wf2_lifecycle.sql przez Supabase
// Management API (POST /v1/projects/<ref>/database/query). Wzorzec:
// apply-wf2-portal-migration.mjs (db push wybucha na rozjechanej historii CLI).
//
// Dodaje kolumnę wf2_projects.lifecycle (active|paused|cancelled|completed) i
// backfilluje 'completed' dla status='zamkniety'. Idempotentne.
//
...
//
// Uruchom: node scripts/apply-wf2-lifecycle.mjs
...
const die = (m) => { console.error(`[apply-wf2-lifecycle] BŁĄD: ${m}`); process.exit(1); };
const log = (m) => console.log(`[apply-wf2-lifecycle] ${m}`);
...
(async () => {
  const mig = readFileSync(join(ROOT, 'supabase', 'migrations', '20260721_wf2_lifecycle.sql'), 'utf8');
  log('Aplikuję 20260721_wf2_lifecycle.sql …');
  await sql(mig);
  log('  ✓ zastosowana');
  ...
  const col = await sql(
    `SELECT column_name, data_type, column_default, is_nullable
      ...
      WHERE table_schema='public' AND table_name='wf2_projects' AND column_name='lifecycle'`);
  const dist = await sql(
    `SELECT lifecycle, count(*) c FROM public.wf2_projects GROUP BY lifecycle ORDER BY lifecycle`);
  const chk = await sql(
    `SELECT 1 FROM pg_constraint
      WHERE conrelid='public.wf2_projects'::regclass AND contype='c'
        AND pg_get_constraintdef(oid) ILIKE '%lifecycle%'`);

  console.log('\n=== WERYFIKACJA ===');
  console.log('kolumna lifecycle:', JSON.stringify(col[0] || '(BRAK!)'));
  console.log('CHECK na lifecycle:', chk.length ? 'ISTNIEJE' : 'BRAK');
  console.log('rozkład wartości:', JSON.stringify(dist));
  if (!col.length) die('kolumna lifecycle NIE powstała');
  log('gotowe.');
})();
