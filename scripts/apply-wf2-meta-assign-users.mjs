#!/usr/bin/env node
// apply-wf2-meta-assign-users.mjs — aplikuje 20260722n_wf2_meta_assign_users.sql (settings key
// wf2_meta_assign_users = lista userów Meta do przypisywania kont pod widoczność MCP) przez
...
//
// Uruchom: node scripts/apply-wf2-meta-assign-users.mjs
...
const die = (m) => { console.error(`[apply-wf2-meta-assign-users] BŁĄD: ${m}`); process.exit(1); };
const log = (m) => console.log(`[apply-wf2-meta-assign-users] ${m}`);
...
(async () => {
  const mig = readFileSync(join(ROOT, 'supabase', 'migrations', '20260722n_wf2_meta_assign_users.sql'), 'utf8');
  log('Aplikuję 20260722n_wf2_meta_assign_users.sql …');
  await sql(mig);
  log('  ✓ zastosowana');

  const row = await sql(`SELECT key FROM public.settings WHERE key='wf2_meta_assign_users'`);
  console.log('\n=== WERYFIKACJA ===');
  console.log('settings.wf2_meta_assign_users:', JSON.stringify(row[0] || '(BRAK!)'));
  if (!row[0]) die('klucz wf2_meta_assign_users NIE powstał');
  log('gotowe — klucz zweryfikowany.');
})();
