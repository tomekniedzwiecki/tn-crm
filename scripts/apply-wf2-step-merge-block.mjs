#!/usr/bin/env node
// apply-wf2-step-merge-block.mjs — aplikuje 20260722l_wf2_step_merge_block.sql (rozszerza wf2_step_merge
// o parametr p_block_merge) przez Supabase Management API (POST /v1/projects/<ref>/database/query).
...
//
// Uruchom: node scripts/apply-wf2-step-merge-block.mjs
...
const die = (m) => { console.error(`[apply-wf2-step-merge-block] BŁĄD: ${m}`); process.exit(1); };
const log = (m) => console.log(`[apply-wf2-step-merge-block] ${m}`);
...
(async () => {
  const mig = readFileSync(join(ROOT, 'supabase', 'migrations', '20260722l_wf2_step_merge_block.sql'), 'utf8');
  log('Aplikuję 20260722l_wf2_step_merge_block.sql …');
  await sql(mig);
  log('  ✓ zastosowana');

  // po DROP+CREATE ma zostać DOKŁADNIE JEDNA funkcja wf2_step_merge, o 5 argumentach
  const fn = await sql(`SELECT proname, pronargs FROM pg_proc WHERE proname='wf2_step_merge'`);
  console.log('\n=== WERYFIKACJA ===');
  console.log('funkcje wf2_step_merge:', JSON.stringify(fn));
  if (!Array.isArray(fn) || fn.length !== 1) die(`oczekiwano 1 funkcji wf2_step_merge, jest ${Array.isArray(fn) ? fn.length : '?'} (ambiguity!)`);
  if (Number(fn[0].pronargs) !== 5) die(`pronargs=${fn[0].pronargs} (oczekiwano 5)`);
  log('gotowe — funkcja zweryfikowana (1×, 5 argumentów).');
})();
