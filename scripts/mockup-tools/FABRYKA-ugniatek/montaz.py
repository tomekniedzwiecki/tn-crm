# -*- coding: utf-8 -*-
"""F4 MONTAZ modulow kanonicznych do index.html Ugniatka:
- SEKCJA:08-zamow = skorka aliasow --zc-* + CALY modul checkout-inline@2 (mechanika NIETYKALNA)
- pay-badges kanon w sekcji final (marker zgubiony przez gpt -> wstawka po .fn-micro)
- FOOTER@1 (skorka: font-display, favicon, BEZ ratingu - zakaz social-proof liczbowego)
- STICKY-BUY@1 (IO na .hero) + klasa `hero` na sekcji hero
"""
import io, re, sys
...
# ── wytnij bloki modulu checkout-inline@2 ──
...
# SKORKA markupu (kontrakt: eyebrow/h2/merchant = wolno; mechanika nietykalna)
m_markup = m_markup.replace('<section id="zamow" class="zc-checkout" aria-labelledby="zc-title"',
                            '<section id="zamow" class="zc-checkout" aria-labelledby="zc-title" data-zc-layout="steps"')
m_markup = m_markup.replace('>Dokończ zamówienie<', '>Zamów Ugniatka<')
...
SKIN = '''<!-- ══ SKÓRKA modułu checkout-inline@2 pod Ugniatka (tokeny-aliasy; mechanika NIETYKALNA) ══ -->
<style>
  #zamow.zc-checkout{
    --zc-bg:var(--paper-2); --zc-card:var(--card); --zc-border:var(--line);
    --zc-text:var(--ink); --zc-muted:var(--body);
    --zc-accent:var(--cta); --zc-accent-text:var(--cta-ink);
    --zc-radius:var(--radius-lg); --zc-radius-sm:var(--radius-sm); --zc-radius-cta:var(--radius-sm);
    --zc-shadow:var(--shadow-lg); --zc-font:var(--font-text);
    /* --zc-ok / --zc-error: dziedziczą domyślne modułu (zero nowych hexów w skórce) */
  }
...
sekcja08 = SKIN + '\n' + m_style + '\n\n' + m_markup + '\n\n' + m_script
pat08 = re.compile(r'(<!-- SEKCJA:08-zamow START -->).*?(<!-- SEKCJA:08-zamow END -->)', re.S)
assert pat08.search(idx), 'brak markerow 08'
idx = pat08.sub(lambda m: m.group(1) + '\n' + sekcja08 + '\n' + m.group(2), idx)
print('OK sekcja 08 = checkout-inline@2 (%d znakow)' % len(sekcja08))


# ── pay-badges kanon: CSS globalny + markup (generator z unikalnym id gradientu BLIK) ──
def paybadges(uid):
...
PB_CSS = '''<style>
  /* ── PAY-BADGES (kanoniczny blok — BLIK/Visa/Mastercard/COD, SVG inline) ── */
  ...
  #final .pay-badges{justify-content:center;margin-top:18px}
</style>
'''

# final: gpt zgubil marker PAYBADGES -> wstaw po akapicie .fn-micro (wewnatrz sekcji final)
mfin = re.search(r'(<p class="fn-micro">.*?</p>)', idx, re.S)
assert mfin, 'brak .fn-micro w sekcji final'
idx = idx.replace(mfin.group(1), mfin.group(1) + '\n\n        ' + paybadges('F1'), 1)
print('OK pay-badges w #final')

# ── FOOTER@1 (skorka Ugniatka; BEZ .foot-rating — zakaz social-proof liczbowego) ──
FOOTER = PB_CSS + '''<style>
  ...
  <div class="wrap">
    <div class="foot-top">
      <div class="foot-brandcol">
        ...
        <p class="foot-claim">Płaski masażer z sześcioma głowicami kulowymi — dociskasz oburącz albo kładziesz i się opierasz.</p>
      </div>
    ...
    <div class="foot-bottom">
      <p class="foot-copy">© 2026 Ugniatek · Wszystkie prawa zastrzeżone</p>
...
# ── STICKY-BUY@1 (skorka tokenami Ugniatka; mechanika/IO nietykalne) ──
STICKY = '''<style>
  .sticky-buy{position:fixed;left:0;right:0;bottom:0;z-index:60;transform:translateY(120%);
    transition:transform .32s cubic-bezier(.4,0,.2,1);background:rgba(251,252,252,.95);
    backdrop-filter:blur(12px);border-top:1px solid var(--line);box-shadow:0 -8px 30px rgba(20,40,45,.10);
...
<div class="sticky-buy" role="region" aria-label="Szybkie zamówienie">
  <div class="sb">
    <img class="sb-thumb" src="''' + A + '''assets/packshot-34.webp" width="120" height="120" loading="lazy" alt="Ugniatek">
    <div class="sb-info">
      <div class="sb-price">Ugniatek · <b data-price>189,00 zł</b></div>
...
'''
pats = re.compile(r'<div class="sticky-buy" hidden.*?</div>', re.S)
assert pats.search(idx), 'brak szczatkowego sticky-buy'
idx = pats.sub(lambda m: STICKY, idx, count=1)
print('OK sticky-buy@1')

# klasa .hero (wymog IO modulu sticky-buy)
idx = idx.replace('<section id="hero" class="hr-hero">', '<section id="hero" class="hr-hero hero">', 1)
