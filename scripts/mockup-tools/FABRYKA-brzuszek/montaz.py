# -*- coding: utf-8 -*-
"""F4 MONTAZ modulow kanonicznych do index.html BRZUSZKA:
- SEKCJA:10-zamow: marker <!--CHECKOUT-INLINE--> -> CALY modul checkout-inline@2
  (skorka aliasow --zc-*; mechanika NIETYKALNA; submit = 'Zamawiam z obowiazkiem zaplaty';
   FIX vs Rozmrozik: data-zc-api + data-zc-thumb-src + data-zc-delivery-days na rootcie —
   bez nich buildConfig = null i checkout nie laduje dostawy/sumy)
- pay-badges kanon (markery <!--PAYBADGES--> w hero i zamow; CSS raz)
- SEKCJA:05-wideo: CALA sekcja -> modul wideo-rail@1 z 3 kaflami tt1..tt3 (repeat(3,1fr))
- FOOTER@1 (skorka Brzuszka; BEZ ratingu)
...
A = PUB + 'bud-assets/brzuszek/'
LANDING_API = 'https://yxmavwkwnfuphjqbelws.supabase.co/functions/v1/wf2-landing-api'
PACKSHOT = A + 'assets/packshot-alpha.png'
...
# FIX roota: w montazu #zamow to sekcja GPT (scopuje CSS `#zamow .zm-*`, bez data-zc-*),
# a atrybuty konfiguracji sa na wewnetrznym div.zc-checkout. Modul czyta root przez
# getElementById('zamow') -> trafialby w sekcje bez data-zc-* -> buildConfig null -> guard.
# Przekierowujemy root na element .zc-checkout (mechanika walidacji/flow bez zmian).
assert "document.getElementById('zamow')" in m_script, 'brak root getElementById w module'
m_script = m_script.replace("var root = document.getElementById('zamow')",
                            "var root = document.querySelector('.zc-checkout')")

inner = re.search(r'<section id="zamow"[^>]*>(.*)</section>\s*$', m_markup, re.S).group(1)
# FIX vs Rozmrozik: przenosimy data-zc-api + dokladamy thumb-src + delivery-days.
m_markup2 = ('<div class="zc-checkout" data-zc-layout="steps"'
             ' data-zc-product="{{WF2_PRODUCT_ID}}"'
             ' data-zc-api="' + LANDING_API + '"'
             ' data-zc-thumb-src="' + PACKSHOT + '"'
             ' data-zc-delivery-days="2">'
             + inner + '</div>')
...
SKIN = '''<!-- SKORKA modulu checkout-inline@2 pod Brzuszka (aliasy tokenow; mechanika NIETYKALNA) -->
<style>
  #zamow .zc-checkout{
    ...
    --zc-step-num-bg:var(--ink); --zc-step-num-text:#FFFFFF;
  }
  #zamow .zc-checkout .zc-head{display:none}   /* naglowek sekcji (nasz H2) pelni te role — makieta bez "Dokoncz zamowienie" */
  #zamow .zc-checkout .zc-eyebrow{font-family:var(--font-text)}
  #zamow .zc-checkout .zc-h2{font-family:var(--font-display);font-stretch:125%;font-weight:800;letter-spacing:-.02em}
  #zamow .zc-checkout .zc-cta,#zamow .zc-checkout button[type="submit"]{font-family:var(--font-display);font-stretch:125%}
</style>
'''
...
print('OK checkout-inline@2 w karcie #zamow (%d znakow); data-zc-api+thumb+delivery-days dodane' % len(blok))
...
# ── WIDEO-RAIL@1: cala sekcja 05 -> modul z 3 kaflami ──
# skora Brzuszka: border kafla -> --line; grid repeat(4)->repeat(3); usun dekoracyjny vid__swash
rail_sec = re.search(r'(<section id="wideo".*?</section>)', rail, re.S).group(1)
rail_sec = rail_sec.replace('border: 1px solid rgba(241, 87, 58, .2);', 'border: 1px solid var(--line);')
rail_sec = rail_sec.replace('grid-template-columns: repeat(4, 1fr);', 'grid-template-columns: repeat(3, 1fr);')
# naglowek: eyebrow + reps signature + swash prosty (usun dekoracyjny SVG swash)
rail_sec = rail_sec.replace('{{EYEBROW}}', 'ZOBACZ RUCH')
rail_sec = rail_sec.replace(
    '<h2 id="vid-h" class="h2 vid__title">{{TYTUL}}</h2>',
    '<h2 id="vid-h" class="h2 vid__title">Najpierw zobacz. Potem ustaw po <span class="swash">swojemu</span>.</h2>')
rail_sec = re.sub(r'<svg class="vid__swash".*?</svg>', '', rail_sec, flags=re.S)
rail_sec = rail_sec.replace('{{PODTYTUL}}', 'Krótkie pionowe klipy pokazują biało-różową maszynę w prawdziwych wnętrzach.')
# reps signature po eyebrow
rail_sec = rail_sec.replace(
    '<p class="eyebrow vid__eyebrow">ZOBACZ RUCH</p>',
    '<p class="eyebrow vid__eyebrow">ZOBACZ RUCH</p>\n      <span class="reps" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></span>')

# 3 kafle
tile_tpl = re.search(r'(<li class="vid__tile".*?</li>)', rail_sec, re.S).group(1)
KLIPY = [
    ('tt1', '@eves444x', 'Brzuszek w użyciu', 'Kobieta ćwiczy crunch na biało-różowej maszynie w salonie'),
    ('tt2', '@gina.ruhnay', 'Wiele ćwiczeń w domu', 'Biało-różowa maszyna w użyciu w domowym pokoju'),
    ('tt3', '@relifefitnessus', 'Demo: ćwiczenia core', 'Prezentacja ćwiczeń na biało-różowej maszynie'),
]
tiles, dots = [], []
...
# podmien liste kafli (jeden szablonowy li -> 3) i kropki (jeden li active -> 3)
rail_sec = rail_sec.replace(tile_tpl, '\n'.join(tiles), 1)
dot_tpl = re.search(r'<button class="vid__dot is-active"[^>]*></button>', rail_sec).group(0)
rail_sec = rail_sec.replace(dot_tpl, '\n      '.join(dots), 1)
# link CTA pod railem -> #wiele-partii (ink, nie cta)
rail_sec = rail_sec.replace('</ul>\n', '''</ul>
    <p style="margin:26px 0 0;text-align:center">
      <a href="#wiele-partii" style="color:var(--ink);font-weight:600;text-decoration:underline;text-underline-offset:4px">Sprawdź, co możesz ćwiczyć →</a>
    </p>
''', 1)

pat_w = re.compile(r'(<!--\s*SEKCJA:05-wideo START\s*-->).*?(<!--\s*SEKCJA:05-wideo END\s*-->)', re.S)
assert pat_w.search(idx), 'brak markerow 05-wideo'
idx = pat_w.sub(lambda m: m.group(1) + '\n' + rail_sec + '\n' + m.group(2), idx)
assert idx.count('{{MP4_') == 0 and '{{TYTUL}}' not in idx, 'niepodmienione placeholdery wideo'
print('OK wideo-rail@1 (3 kafle, repeat(3,1fr))')
...
FOOTER = '''<style>
  ...
  #footer .foot-brand .brand-mark{height:34px;width:auto;display:block;flex:0 0 auto;border-radius:8px}
  #footer .foot-brand span{font-family:var(--font-display);font-stretch:125%;font-weight:800;font-size:26px;letter-spacing:-.01em}
  #footer .foot-claim{margin-top:15px;font-size:14.5px;color:var(--body);line-height:1.55;max-width:340px}
  #footer .foot-h{font-family:var(--font-display);font-stretch:125%;font-weight:700;font-size:15px;color:var(--ink);letter-spacing:.02em;margin-bottom:16px}
  ...
  <div class="wrap">
    <div class="foot-top">
      <div class="foot-brandcol">
        ...
        <p class="foot-claim">Składana maszyna do ćwiczeń brzucha i core: ruchomy wózek na szynie, 5 wysokości, 2 kąty nachylenia i licznik LCD. Ustaw poziom, zrób swoją serię, złóż.</p>
      </div>
    ...
    <div class="foot-bottom">
      <p class="foot-copy">© 2026 Brzuszek · Wszystkie prawa zastrzeżone</p>
...
STICKY = '''<style>
  .sticky-buy{position:fixed;left:0;right:0;bottom:0;z-index:60;transform:translateY(120%);
    transition:transform .32s cubic-bezier(.4,0,.2,1);background:rgba(247,245,251,.95);
    backdrop-filter:blur(12px);border-top:1px solid var(--line);box-shadow:0 -8px 30px rgba(45,35,60,.10);
  ...
  .sticky-buy .sb-thumb{width:44px;height:44px;border-radius:var(--radius-sm);object-fit:contain;flex:0 0 auto;background:#FFFFFF;border:1px solid var(--line)}
  .sticky-buy .sb-info{flex:1;min-width:0}
  .sticky-buy .sb-price{font-weight:700;font-size:16.5px;color:var(--ink)}
  .sticky-buy .sb-price b{font-family:var(--font-display);font-stretch:125%;font-weight:800}
...
<div class="sticky-buy" role="region" aria-label="Szybkie zamówienie">
  <div class="sb">
    <img class="sb-thumb" src="''' + A + '''assets/packshot-thumb.webp" width="120" height="120" loading="lazy" alt="Brzuszek">
    <div class="sb-info">
      <div class="sb-price">Brzuszek · <b data-price>429,00 zł</b></div>
