# -*- coding: utf-8 -*-
"""F4 MONTAZ modulow kanonicznych do index.html ODSACZKA:
- SEKCJA:08-zamow: marker <!--CHECKOUT-INLINE--> w karcie checkout -> CALY modul
  checkout-inline@2 (skorka aliasow --zc-*; mechanika NIETYKALNA)
- pay-badges kanon (id gradientow Zc/F1/F2 unikalne)
- FOOTER@1 (skorka Odsaczka; BEZ ratingu)
- STICKY-BUY@1 (IO na .hero; thumb=packshot-thumb)
...
A = 'https://yxmavwkwnfuphjqbelws.supabase.co/storage/v1/object/public/attachments/bud-assets/odsaczek/'
...
# modul ma wlasny <section id="zamow"> — sekcja gpt juz jest w indexie; modul wchodzi
# JAKO WNETRZE karty checkout (marker CHECKOUT-INLINE). Zdejmij zewnetrzny section wrapper.
inner = re.search(r'<section id="zamow"[^>]*>(.*)</section>\s*$', m_markup, re.S).group(1)
# ale mechanika CSS modulu celuje w .zc-checkout — wiec owin w div.zc-checkout (alias)
m_markup2 = ('<div class="zc-checkout" data-zc-layout="steps" data-zc-product="{{WF2_PRODUCT_ID}}">'
             + inner + '</div>')
m_markup2 = m_markup2.replace('>Dokończ zamówienie<', '>Zamów Odsączka<')
...
SKIN = '''<!-- SKORKA modulu checkout-inline@2 pod Odsaczka (aliasy tokenow; mechanika NIETYKALNA) -->
<style>
  #zamow .zc-checkout{
...
assert len(pat_ci.findall(idx)) == 1, 'CHECKOUT-INLINE != 1 wystapienie'
...
# PAYBADGES marker w sekcji zamow
...
idx = pat_pb.sub(lambda m: paybadges('X2'), idx)  # ewentualne kolejne bez dubla CSS
  ...
  <div class="wrap">
    <div class="foot-top">
      <div class="foot-brandcol">
        ...
        <p class="foot-claim">Składany koszyk ze stali nierdzewnej — smażysz, wyjmujesz całą porcję jednym ruchem i zawieszasz na rancie garnka.</p>
      </div>
    ...
    <div class="foot-bottom">
      <p class="foot-copy">© 2026 Odsączek · Wszystkie prawa zastrzeżone</p>
...
STICKY = '''<style>
  .sticky-buy{position:fixed;left:0;right:0;bottom:0;z-index:60;transform:translateY(120%);
    transition:transform .32s cubic-bezier(.4,0,.2,1);background:rgba(255,252,246,.95);
    backdrop-filter:blur(12px);border-top:1px solid var(--line);box-shadow:0 -8px 30px rgba(46,38,24,.10);
...
<div class="sticky-buy" role="region" aria-label="Szybkie zamówienie">
  <div class="sb">
    <img class="sb-thumb" src="''' + A + '''assets/packshot-thumb.webp" width="120" height="120" loading="lazy" alt="Odsączek">
    <div class="sb-info">
      <div class="sb-price">Odsączek · <b data-price>29,90 zł</b></div>
...
'''
pats = re.compile(r'<div class="sticky-buy"[^>]*hidden[^>]*>.*?</div>\s*<!--\s*/STICKY-BUY\s*-->|'
                  r'<!--STICKY-BUY-->', re.S)
m_st = pats.search(idx)
assert m_st, 'brak markera STICKY-BUY'
idx = pats.sub(lambda m: STICKY, idx, count=1)
print('OK sticky-buy@1')

# klasa .hero na sekcji hero (wymog IO)
if 'class="hr-hero hero"' not in idx:
    idx = re.sub(r'<section id="hero" class="([^"]*)"', lambda m:
                 '<section id="hero" class="%s hero"' % m.group(1)
                 if 'hero' not in m.group(1).split() else m.group(0), idx, count=1)
print('OK klasa .hero')
