# -*- coding: utf-8 -*-
"""Hero-loop TLO Skrolika — naprawa 23.07 (audyt): hero byl "ozywiona pocztowka" (.hr-media
w ramce radius+shadow obok tekstu). Nowy kanon: JEDEN kadr full-bleed osadzony w TLE (mata).
sc-hero-tlo = 3:2, dlon z rozowym pierscieniem Skrolik + telefon z przewijanym feedem po PRAWEJ,
LEWA 40% = negative-space pod copy+scrim. Klasa: nosnik ruchu = feed na ekranie telefonu przewija
sie pionowo (dzialanie skrolika) + klik kciukiem; pierscien/dlon/telefon rigid (NEG lock). GATE
diff(0-5s) >= 8.0. Ping-pong. Upload pod NOWA nazwa -tlo. Skin/tokeny Skrolika (--paper #F8F1F0)."""
...
GEN_PNG = os.path.join(SCRATCH, "skrolik-hero", "skrolik-hero-tlo-v1.png")
RESULT = os.path.join(SCRATCH, "skrolik-hero-v4-result.json")
...
PROMPT = ("The social feed on the phone screen scrolls smoothly and continuously upward, rows of "
          "small photo tiles sliding up and being replaced the whole time, clearly visible the whole "
          "time. The thumb wearing the pink ring presses its scroll button with a small gentle "
          "repeating tap. The hand, the pink silicone ring with its two triangle buttons, the phone "
          "body and the cream sofa all stay completely still and unchanged in shape and colour. "
          "Locked-off camera, photorealistic, no colour change.")
NEG = ("camera movement, camera pan, camera zoom, camera shake, ring morphing, ring changing shape "
       "or colour, buttons changing, extra fingers, sixth finger, deformed hand, hand morphing, "
       "melting fingers, phone deforming, phone screen going blank or white, new objects, second "
       "phone, second ring, another hand, another person, text captions, watermark, flicker, glitch, "
       "jerky motion, strobing, split screen, collage, seam, dividing line, colour shift")
...
src_jpg = os.path.join(SCRATCH, "skrolik-hero-tlo-src.jpg")
Image.open(GEN_PNG).convert("RGB").save(src_jpg, "JPEG", quality=92)
upload("bud-assets/skrolik/video/hero-src-tlo.jpg", src_jpg, "image/jpeg")
SRC_URL = BASE + "/storage/v1/object/public/attachments/bud-assets/skrolik/video/hero-src-tlo.jpg"
...
rawp = os.path.join(SCRATCH, "skrolik-tlo-raw.mp4")
open(rawp, "wb").write(urllib.request.urlopen(vurl, timeout=300).read())
pp = os.path.join(SCRATCH, "skrolik-tlo-pp.mp4")
...
fr = {}
for t in ("0", "2.5", "5"):
    fp = os.path.join(SCRATCH, f"skrolik-tlo-f{t}.jpg")
...
poster_raw = os.path.join(SCRATCH, "skrolik-tlo-poster.jpg")
subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-i", rawp,
    "-frames:v", "1", "-qscale:v", "3", poster_raw], check=True)
pim = Image.open(poster_raw).convert("RGB")
p1536 = os.path.join(SCRATCH, "skrolik-sc-hero-tlo.webp")
pim.save(p1536, "WEBP", quality=84)
p800 = os.path.join(SCRATCH, "skrolik-sc-hero-tlo-800.webp")
w, h = pim.size
pim.resize((800, round(800 * h / w))).save(p800, "WEBP", quality=82)
upload("bud-assets/skrolik/assets/sc-hero-tlo.webp", p1536, "image/webp")
upload("bud-assets/skrolik/assets/sc-hero-tlo-800.webp", p800, "image/webp")
upload("bud-assets/skrolik/video/hero-loop-tlo.mp4", pp, "video/mp4")
upload("bud-assets/skrolik/video/hero-loop-tlo-poster.webp", p1536, "image/webp")
