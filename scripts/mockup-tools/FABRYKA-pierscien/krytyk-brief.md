Jesteś KRYTYKIEM przewodnika graficznego fabryki landingów (gate F1.7). Oceń przewodnik
poniżej OSTRO i konkretnie. Pisz PO POLSKU. Sprawdź:
1. OSIE RÓŻNORODNOŚCI: ≥3 konteksty, ≥3 skale, ≥2 światła, człowiek ≥30% scen — czy mapa
   jest PRAWDZIWA względem opisów scen (nie tylko zadeklarowana)?
2. ANTY-SZEW: sąsiednie sekcje nie dzielą kontekst+skala.
3. WIERNOŚĆ: czy każdy seed niesie pełną tożsamość produktu (klinowaty blok, DOKŁADNIE 3
   przyciski w skosie, otwarty klips C, różowy mat, gniazdo+pinhole) + ORIENT + NEG?
4. WYKONALNOŚĆ: elementy ryzykowne dla modelu obrazkowego (dłonie, ekrany, łuki graficzne)
   — czy przewodnik je adresuje?
5. ZAKAZY: brak twarzy w zbliżeniu, brak nocy/siłowni/biura, ekrany nieczytelne, zero
   tekstu w obrazie.
FORMAT: `WERDYKT: PASS` albo `WERDYKT: FAIL` w pierwszej linii, potem max 8 punktów uwag
(⛔ blokujące / ⚠ zalecenia). Zwięźle.
...
ORIENT: ring worn on the index finger, keystone block centered on top of the finger, sloped three-button face angled toward the thumb, thumb placed on the middle button, open C-clip and underside split visibly wrapping under the finger, top and one side shown in a clear three-quarter view.

NEGATIVE PROMPT: no screen or display on the ring, no fourth button, no button count other than exactly three, no metal on the ring, no metallic finish, no closed ring band, no dial, no wheel, no roller, no RGB or product illumination, no logos or writing on the product, no cable connected to the ring, no charging station or charging dock for the ring, no strap on the ring, no black product, no cream product, no product color other than pastel matte pink, no readable text, no recognizable app or UI, no app logos, no watermark, no faces in close-up, no night scene, no dark living room, no gym, no office, no restaurant, no gamer-RGB, no stock studio setup, no deformed hands, no extra fingers, no duplicated product.
```
...
ORIENT: ring worn on the index finger, keystone block resting on top of the finger, three-button face turned inward toward the thumb, thumb relaxed beside the buttons, open C-clip and split visible beneath the finger against the blanket, product shown in a natural top-side three-quarter angle.
...
ORIENT: ring worn on the index finger of the free hand, keystone block centered above the finger, sloped three-button face directly accessible to the same hand’s thumb, thumb gently pressing the middle button, open C-clip and underside split visible below the index finger, clear top-side three-quarter product view.
...
```text
Photorealistic square 1:1 lifestyle image in a bright reading corner beside a window. An adult in ordinary light home clothing sits in a pale armchair or on a wide window sill, framed without a visible face. A smartphone or tablet stands in front of the person at a comfortable reading distance. Its screen shows a neutral ebook-like page made only of heavily blurred soft gray paragraph lines, with no readable text, title, recognizable app, icons, logos or branding.
...
- **Beat 5 s (kamera STATYCZNA, pętla first=last):** 0,0–0,8 s bezruch (ustalenie kadru);
  ...
  napina); 1,2–4,2 s zawartość ekranu telefonu PŁYNIE pionowo w górę (miękkie rozmyte
  bloki feedu, ruch jednostajny, 1,5–2 przesunięcia „kart"); 4,2–5,0 s ekran wyhamowuje,
  kciuk wraca do spoczynku — klatka końcowa = początkowa. Reszta kadru (dłoń, koc, okno,
...
- **Prompt (i2v):** "Static camera. The thumb gently presses the middle button of the pink
  ...
  screen scrolls smoothly upward by two content blocks, then eases to a stop. The hand,
...
| 2 | sc-kanapa | 03 ekran-zostaje | ekran telefonu na stoliku — feed płynie pionowo; dłoń/koc statyczne | klik-mikro kciuka → 2 bloki przesuwu → stop (pętla first=last) |
| 3 | sc-kuchnia | 03b/kadr B | ekran tabletu z przepisem — treść przesuwa się o 1 sekcję; para z miski delikatnie | kciuk dociska → przepis płynie → stop; dłonie przy misce NIERUCHOME |
