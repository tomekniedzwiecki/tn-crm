# -*- coding: utf-8 -*-
"""Hero-loopy ZAKLIPEK (Kling PRO i2v) — F5 lp_zycie. Cinemagraph: produkt + dłoń STATYCZNE,
ruch tylko ambient (para z kubka, roślina/firana, chłodne światło). 2 klipy: desktop (sc-hero-d)
+ mobile (sc-hero-m). Wynik: video/hero-loop{,-m}.{mp4,webm} + -poster.webp + assets/hero-video.mp4
(kafel home). Nazwy 1:1 z injektorem landingu (#hero .hero-video-inject: hero-loop / hero-loop-m)."""
...
NEG = ("camera movement, camera pan, camera zoom, people moving, person moving, hand moving, "
       "face, fingers moving, product deforming or morphing, USB hub moving, object color change, "
       "glow, text, captions, watermark, flicker, glitch, fast motion, new objects appearing")

ASSETS = BASE + "/storage/v1/object/public/attachments/bud-assets/zaklipek/assets/"
PROMPT = ("Cinemagraph-style very subtle looping motion: a thin wisp of steam rises slowly from "
          "the coffee mug at the edge of the desk and gently dissolves; the leaves of the small "
          "houseplant by the window tremble almost imperceptibly and the cool daylight breathes "
          "softly. The silver aluminium USB hub clamped on the desk edge AND the hand holding the "
          "USB stick stay PERFECTLY still, frozen like a photograph — no finger or hand motion. "
          "Locked-off camera, photorealistic, calm seamless loop.")
...
for c in CLIPS:
    ...
    print(f"[{c['out']}] size={size} -> request {rid or sub}")
    if rid:
...
while pending and time.time() - t0 < 1500:
    ...
    for j in list(pending):
        ...
        print(f"  poll {j['out']}: {s}  (+{int(time.time()-t0)}s)")
        if s == "COMPLETED":
            ...
            rawp = os.path.join(LOCAL, f"{SLUG}-{j['out']}-raw.mp4")
            ...
            subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-i", pp,
                "-c:v", "libvpx-vp9", "-crf", "34", "-b:v", "0", "-an", "-row-mt", "1", webm], check=True)
            poster = os.path.join(LOCAL, f"{SLUG}-{j['out']}-poster.jpg")
            subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-i", rawp,
                "-frames:v", "1", "-qscale:v", "3", poster], check=True)
            from PIL import Image
            wp = poster.replace(".jpg", ".webp")
            Image.open(poster).save(wp, "WEBP", quality=80)
            ...
            if j.get("home"):
                upload(f"bud-assets/{SLUG}/assets/hero-video.mp4", ppd, "video/mp4")
            print(f"[{j['out']}] DONE mp4={len(ppd)//1024}KB webm={len(wbd)//1024}KB -> video/{j['out']}.{{mp4,webm}}"
                  + (" + assets/hero-video.mp4" if j.get("home") else ""))
            pending.remove(j)
        elif s in ("FAILED", "ERROR"):
            print(f"[{j['out']}] FAILED -> {proxy('poll', url=j['response_url'])}"); pending.remove(j)
for j in pending:
    print(f"[{j['out']}] TIMEOUT — request_id={j['rid']}")
print("KONIEC gen-hero-zaklipek")
