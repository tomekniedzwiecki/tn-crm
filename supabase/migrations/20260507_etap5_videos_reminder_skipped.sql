-- =============================================
-- EMAIL TEMPLATES: videos_reminder + videos_skipped
-- Etap 5/Krok 3 — przypomnienie o nagraniach albo pominiecie etapu
-- =============================================
...
INSERT INTO settings (key, value) VALUES (
  'email_template_videos_reminder_subject',
  'Przypomnienie: czekamy na Twoje nagrania video 🎬'
) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;

INSERT INTO settings (key, value) VALUES (
  'email_template_videos_reminder_body',
...
-- ─── VIDEOS_SKIPPED ──────────────────────────────
INSERT INTO settings (key, value) VALUES (
  'email_template_videos_skipped_subject',
  'Etap nagrań video pominięty — idziemy dalej'
) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;

INSERT INTO settings (key, value) VALUES (
  'email_template_videos_skipped_body',
...
DECLARE
  v_flow_id UUID;
BEGIN
  -- videos_reminder flow
  SELECT id INTO v_flow_id FROM automation_flows WHERE trigger_type = 'videos_reminder' LIMIT 1;
  IF v_flow_id IS NULL THEN
    INSERT INTO automation_flows (name, description, trigger_type, trigger_filters, is_active)
    VALUES ('Etap 5/Krok 3 — Przypomnienie o video', 'Email do klienta gdy admin chce przypomniec o nagraniach video', 'videos_reminder', '{}', true)
    RETURNING id INTO v_flow_id;
    INSERT INTO automation_steps (flow_id, step_type, step_order, config)
    VALUES (v_flow_id, 'action', 0, '{"action_type": "send_email", "email_type": "videos_reminder"}');
  END IF;

  -- videos_skipped flow
  SELECT id INTO v_flow_id FROM automation_flows WHERE trigger_type = 'videos_skipped' LIMIT 1;
  IF v_flow_id IS NULL THEN
    INSERT INTO automation_flows (name, description, trigger_type, trigger_filters, is_active)
    VALUES ('Etap 5/Krok 3 — Video pominiete', 'Email do klienta gdy admin oznaczy etap video jako pominiety', 'videos_skipped', '{}', true)
    RETURNING id INTO v_flow_id;
    INSERT INTO automation_steps (flow_id, step_type, step_order, config)
    VALUES (v_flow_id, 'action', 0, '{"action_type": "send_email", "email_type": "videos_skipped"}');
  END IF;
END $$;
