-- Fix: Poprawne tworzenie workflow dla płatności ratalnych
-- Gdy płatność jest ratą, użyj "Budowa sklepu pełen pakiet" jako offer_name
...
BEGIN
    ...
    IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
        ...
        -- Dla rat użyj stałej nazwy, dla normalnych płatności użyj description
...
COMMENT ON FUNCTION create_workflow_on_payment() IS 'Automatycznie tworzy workflow gdy order.status zmienia się na paid. Dla płatności rat używa "Budowa sklepu pełen pakiet" jako offer_name.';
