-- Fix: Trigger SQL powinien wywoływać automation-trigger zamiast send-email bezpośrednio
-- Dzięki temu automatyzacje (workflow_created) będą działać poprawnie
...
BEGIN
    ...
    IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
        ...
        -- Wywołaj automation-trigger zamiast send-email bezpośrednio
        -- Dzięki temu automatyzacje będą działać poprawnie
...
DROP TRIGGER IF EXISTS workflow_on_payment_trigger ON orders;
CREATE TRIGGER workflow_on_payment_trigger
    AFTER UPDATE ON orders
    FOR EACH ROW
    EXECUTE FUNCTION create_workflow_on_payment();

COMMENT ON FUNCTION create_workflow_on_payment() IS 'Automatycznie tworzy workflow gdy order.status zmienia się na paid i wywołuje automation-trigger';
