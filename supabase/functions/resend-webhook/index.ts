...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify webhook signature (if secret is configured). Akceptuj dowolny z
    // skonfigurowanych sekretów — odporność na pomyłkę „który sekret gdzie".
    const candidateSecrets = [
      Deno.env.get('RESEND_WEBHOOK_SECRET_TRACKING'),
    ...
    if (candidateSecrets.length) {
      ...
      for (const secret of candidateSecrets) {
        if (await verifySvixSignature(svixId, bodyText, signature, timestamp, secret)) { isValid = true; break }
      }
      if (!isValid) {
        console.error(`[resend-webhook] Invalid signature - rejecting (tried ${candidateSecrets.length} secret(s))`)
    ...
    // Stempluj też spar_emails. UWAGA: otwarcia/kliknięcia są zwykle WYŁĄCZONE
    // w Resend (niewiarygodne) — realnie napędza decyzje tylko `delivered_at`
    // (pewny sygnał „adres żyje"). opened/clicked zostają dormantnie na wypadek
    // ponownego włączenia, ale logika dripa/followups NIE może na nich polegać.
    try {
      if (payload.type === 'email.delivered') {
        await supabase.from('spar_emails').update({ delivered_at: new Date().toISOString() })
          .eq('resend_id', resendId).is('delivered_at', null)
      } else if (payload.type === 'email.opened') {
        await supabase.from('spar_emails').update({ opened_at: new Date().toISOString() })
          .eq('resend_id', resendId).is('opened_at', null)
      } else if (payload.type === 'email.clicked') {
        await supabase.from('spar_emails').update({ clicked_at: new Date().toISOString() })
