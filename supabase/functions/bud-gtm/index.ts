// bud-gtm — go-to-market e-commerce: playbook pierwszych klientów + pakiet
// startowy (gotowe reklamy z copy + posty + maile powitalne). Lejek "Zbuduję /
// Sklep" (AWE — e-commerce fizyczny; tomekniedzwiecki.pl/sklep). Model gpt-5.5.
//
// ⚠️ DEPLOY: ZAWSZE z flagą --no-verify-jwt:
//   npx supabase functions deploy bud-gtm --no-verify-jwt
//
...
// odpowiedź (jakość: dokładnie 4 reklamy + nagłówki ≤10 słów). Limit 4 gen/sesja.
...
const OPENAI_MODEL = Deno.env.get('BUD_GTM_MODEL') || 'gpt-5.5'
...
// Banery reklam — gpt-image-2 (jak bud-image). Koszt per quality do bud_usage.
const STORAGE_BUCKET = 'attachments'
const IMAGE_MODEL = Deno.env.get('BUD_IMAGE_MODEL') || 'gpt-image-2'
const IMAGE_MODEL_FALLBACK = 'gpt-image-1'
const IMAGE_QUALITY = Deno.env.get('BUD_GTM_BANNER_QUALITY') || 'medium'
...
// Prompt kreacji reklamowej — kreacja BUDOWANA WOKÓŁ realnej wizualizacji sklepu
// (ekran/produkt idzie jako obraz wejściowy do /images/edits; tu tylko instrukcja kompozycji)
function buildBannerPrompt(reklama: Record<string, unknown>, brief: Record<string, unknown>, nazwa: string): string {
  ...
  return [
    `Stwórz PROFESJONALNĄ KREACJĘ REKLAMOWĄ (baner social media) dla polskiego sklepu internetowego „${nazwa}". Format kanału: ${s(reklama.format, 30) || 'feed 1:1'}.`,
    `Dołączony obraz to PRAWDZIWA wizualizacja produktu / sklepu. Pokaż go WIERNIE i atrakcyjnie — umieść NA PIERWSZYM PLANIE jako bohatera kadru (produkt w czystej, apetycznej aranżacji). NIE przeprojektowuj produktu, nie zmieniaj jego cech, nie dorysowuj fałszywych elementów.`,
    ...
    paleta + `Styl: czysty, premium — jak reklama dobrego sklepu e-commerce; tło spójne z paletą marki, dużo oddechu pod nagłówek. Bez stockowych ludzi (chyba że produkt tego wymaga), bez clipartów, bez znaków wodnych, bez zmyślonego/losowego tekstu. Mocny kontrast, czytelność na małym ekranie telefonu.`,
  ].filter(Boolean).join(' ')
}
// Generacja jednego banera przez /images/edits — REALNA wizualizacja jako obraz wejściowy
...
async function genBanner(apiKey: string, prompt: string, size: string, screenBytes: Uint8Array): Promise<{ bytes: Uint8Array; usedModel: string }> {
  const call = (model: string) => {
    ...
    fd.append('n', '1')
    fd.append('image', new Blob([screenBytes], { type: 'image/png' }), 'wizualizacja.png')
  ...
  if (!res.ok) {
    ...
    console.error(`[bud-gtm] ${IMAGE_MODEL} edit error:`, res.status, errText.slice(0, 300))
    // 400/404 → spróbuj fallbackiem (np. gdy gpt-image-2.0 nie wspiera /images/edits)
    if (res.status === 400 || res.status === 404) { res = await call(IMAGE_MODEL_FALLBACK); usedModel = IMAGE_MODEL_FALLBACK; if (!res.ok) { console.error('[bud-gtm] fallback edit error:', res.status, (await res.text().catch(() => '')).slice(0, 300)); throw new Error('img_error') } }
...
// URL wizualizacji z preview_images (string albo {url})
...
// Tło: generuje do 4 banerów Z REALNYCH WIZUALIZACJI, zapisuje banner_url w gtm.reklamy.
async function generateBanners(supabase: ReturnType<typeof createClient>, apiKey: string, sessionId: string): Promise<void> {
  try {
    const { data: session } = await supabase.from('bud_sessions').select('preview_brief, preview_images, gtm').eq('id', sessionId).maybeSingle()
    if (!session || !session.gtm) { console.error('[bud-gtm] banners: brak gtm'); return }
    const brief = (session.preview_brief || {}) as Record<string, unknown>
    const nazwa = (typeof brief.nazwa === 'string' && brief.nazwa.trim()) ? brief.nazwa.trim() : 'sklep'
    ...
    // realne wizualizacje, w kolejności „najlepsze do reklamy" (produkt > sklep > karta_produktu > lifestyle > landing)
    const imgs = (session.preview_images || {}) as Record<string, unknown>
    const screens = ['produkt', 'sklep', 'karta_produktu', 'lifestyle', 'glowna', 'landing', 'podsumowanie'].map((k) => screenUrl(imgs[k])).filter(Boolean)
    if (!screens.length) { console.error('[bud-gtm] banners: brak wizualizacji — pomijam (edits wymaga obrazu)'); await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_banners' }); return }
    ...
    for (let i = 0; i < reklamy.length && i < 4; i++) {
      ...
      if (!screenBytes) { console.error('[bud-gtm] banner', i + 1, 'nie pobrano wizualizacji — pomijam'); continue }
      try {
        ...
        const path = `bud/${sessionId}/reklama-${i + 1}-${Date.now()}.png`
        const { error: upErr } = await supabase.storage.from(STORAGE_BUCKET).upload(path, bytes, { contentType: 'image/png', upsert: true })
        if (upErr) { console.error('[bud-gtm] banner upload error:', upErr); continue }
        const { data: pub } = supabase.storage.from(STORAGE_BUCKET).getPublicUrl(path)
        if (pub?.publicUrl) {
          r.banner_url = pub.publicUrl; r.banner_size = size; changed = true
          await supabase.from('bud_usage').insert({ session_id: sessionId, kind: 'image', model: usedModel, images: 1, cost_usd: IMAGE_COST_USD[IMAGE_QUALITY] ?? IMAGE_COST_USD.medium, meta: { view: 'ad_banner', idx: i + 1, quality: IMAGE_QUALITY } })
          // zapisujemy po KAŻDYM banerze — frontend widzi je pojawiające się stopniowo
          await supabase.from('bud_sessions').update({ gtm, updated_at: new Date().toISOString() }).eq('id', sessionId)
        }
      } catch (e) { console.error('[bud-gtm] banner', i + 1, 'error:', e instanceof Error ? e.message : String(e)) }
    }
    await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_banners' })
    console.log(`[bud-gtm] banners done ${sessionId} changed=${changed}`)
  } catch (err) {
    console.error('[bud-gtm] banners ERROR:', err instanceof Error ? err.message : String(err))
    await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_banners' })
  }
}
...
function buildUser(brief: Record<string, unknown>, karta: Record<string, unknown>, plan: Record<string, unknown> | null, raport: Record<string, unknown> | null): string {
  ...
  const fakty: string[] = []
  const nazwa = s(brief.nazwa, 80) || 'Sklep'
  fakty.push(`Sklep: „${nazwa}" — ${s(brief.opis)}`)
  ...
  if (list(karta.ekrany)) fakty.push(`Asortyment / kluczowe produkty: ${list(karta.ekrany)}`)
  if (plan && typeof plan.cena === 'number') fakty.push(`Cena produktu: ${plan.cena} ${s(plan.cena_jednostka, 20) || 'zł'}`)
  if (raport) {
...
async function callOnce(apiKey: string, system: string, user: string, maxTokens: number): Promise<{ obj: Record<string, unknown> | null; usage: { i: number; c: number; o: number } | null }> {
  ...
  if (!res.ok) { console.error('[bud-gtm] openai error:', res.status, (await res.text().catch(() => '')).slice(0, 400)); return { obj: null, usage: null } }
  const data = await res.json()
  const u = data?.usage || {}
  ...
  catch { console.error('[bud-gtm] zły JSON, finish:', data?.choices?.[0]?.finish_reason, String(content).slice(0, 200)); return { obj: null, usage } }
}
...
async function logGtmUsage(supabase: ReturnType<typeof createClient>, sessionId: string, usage: { i: number; c: number; o: number } | null): Promise<void> {
  if (!usage) return
  try { const p = PRICES[OPENAI_MODEL] || PRICES['gpt-5.5']; await supabase.from('bud_usage').insert({ session_id: sessionId, kind: 'gtm', model: OPENAI_MODEL, input_tokens: usage.i, cached_tokens: usage.c, output_tokens: usage.o, cost_usd: (Math.max(0, usage.i - usage.c) * p.i + usage.c * p.c + usage.o * p.o) / 1_000_000 }) } catch (uErr) { console.error('[bud-gtm] usage insert error:', uErr) }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // działać jak hasło (lustrzane odbicie bud-chat).
    {
      ...
      const { data: own } = await supabase.from('bud_sessions').select('auth_user_id').eq('id', sessionId).maybeSingle()
    ...
    if (!SYSTEM_PROMPT) { try { const { data: __pd } = await supabase.from('settings').select('key, value').in('key', ['budowanie_prompt_gtm_system', 'budowanie_prompt_gtm_channels', 'budowanie_prompt_gtm_ads']); const __pv = (k: string) => ((__pd || []) as Array<{ key: string; value: string }>).find((r) => r.key === k)?.value || ''; SYSTEM_PROMPT = __pv('budowanie_prompt_gtm_system'); CHANNELS_SYSTEM = __pv('budowanie_prompt_gtm_channels'); ADS_SYSTEM = __pv('budowanie_prompt_gtm_ads') } catch (_e) { /* fallback: puste prompty */ } }

    // ── action 'banners': wygeneruj kreacje reklam w tle (gpt-image-2) ──
    if (body.action === 'banners') {
      const { data: lock } = await supabase.rpc('bud_claim_lock', { p_session: sessionId, p_key: 'gtm_banners', p_ttl_sec: 300 })
    ...
    const { data: session, error: sErr } = await supabase.from('bud_sessions').select('id, preview_brief, problem_summary, business_plan, market_report, gtm').eq('id', sessionId).maybeSingle()
    if (sErr) { console.error('[bud-gtm] session fetch error:', sErr); return jsonResponse({ error: 'blad_serwera' }, 500, cors) }
    ...
    if (body.action === 'regen_channels') {
      ...
      const { data: lockC } = await supabase.rpc('bud_claim_lock', { p_session: sessionId, p_key: 'gtm_ch', p_ttl_sec: 200 })
      ...
      if (!bestC) { await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_ch' }); return jsonResponse({ error: 'blad_generowania' }, 502, cors) }
      ...
      await supabase.from('bud_sessions').update({ gtm: mergedC, updated_at: new Date().toISOString() }).eq('id', sessionId)
      await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_ch' })
    ...
    if (body.action === 'regen_ads') {
      ...
      const { data: lockA } = await supabase.rpc('bud_claim_lock', { p_session: sessionId, p_key: 'gtm_ads', p_ttl_sec: 200 })
      ...
      if (!bestA) { await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_ads' }); return jsonResponse({ error: 'blad_generowania' }, 502, cors) }
      ...
      await supabase.from('bud_sessions').update({ gtm: mergedA, updated_at: new Date().toISOString() }).eq('id', sessionId)
      await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm_ads' })
    ...
    const { data: lock } = await supabase.rpc('bud_claim_lock', { p_session: sessionId, p_key: 'gtm', p_ttl_sec: 240 })
    ...
    // (≠4 reklamy / za długi nagłówek). Trzymamy ostatni POPRAWNY jako fallback.
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (obj && saneGtm(obj)) {
        ...
        if (attempt === 0) console.warn('[bud-gtm] próba 1 poniżej jakości (reklamy/nagłówki) — ponawiam')
      } else if (attempt === 0) {
        console.warn('[bud-gtm] próba 1 niepoprawna — ponawiam')
      }
    }
    if (!best) {
      console.error('[bud-gtm] obie próby nieudane')
      await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm' })
    ...
    const { error: uErr } = await supabase.from('bud_sessions').update({ gtm: toSave, updated_at: new Date().toISOString() }).eq('id', sessionId)
    if (uErr) console.error('[bud-gtm] save error:', uErr)
    await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'gtm' })
    return jsonResponse({ gtm: best, cached: false }, 200, cors)
  } catch (e) { console.error('[bud-gtm] ERROR:', e); return jsonResponse({ error: 'blad_serwera' }, 500, cors) }
})
