...
function bizplanEmailHtml(plan: any): string {
  ...
  return `<div style="margin:22px 0 6px;">` +
    ...
    `<div style="font-size:12.5px;color:#666;margin:0 0 10px;line-height:1.5;">Prognoza specjalisty osadzona w Twoim produkcie i rynku — ile realnie zostaje Tobie na czysto (po koszcie towaru, wysyłki, reklamy i po 20% Tomka):</div>` +
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return jsonResponse({ ok: true, processed, fired, smsSent, followups_enabled: followupsEnabled() }, 200)
  } catch (e) {
