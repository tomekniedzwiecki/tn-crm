// spar-gtm — go-to-market dla lejka „Aplikacja": playbook „pierwszych 50
// klientów" (gdzie są, skrypty, obiekcje) + pakiet startowy sprzedaży
// (3 reklamy z copy + posty organiczne + sekwencja maili powitalnych).
// Zakładka „Sprzedaż od dnia 1" w panelu projektu.
//
// ⚠️ DEPLOY: ZAWSZE z flagą --no-verify-jwt:
//   npx supabase functions deploy spar-gtm --no-verify-jwt
//
// POST { sessionId, force? } -> cached gdy gtm istnieje; inaczej generacja z
// karty + briefu + market_report + business_plan, zapis w spar_sessions.gtm.
// Model gpt-5.5 (jakość copy reklamowego). Limit 4 generacji/sesja.
...
function buildPrompt(brief: Record<string, unknown>, karta: Record<string, unknown>, plan: Record<string, unknown> | null, raport: Record<string, unknown> | null): string {
  ...
  if (raport) {
    if (Array.isArray(raport.konkurenci) && raport.konkurenci.length) {
      const k = (raport.konkurenci as Record<string, unknown>[]).slice(0, 4).map((c) => s(c.nazwa, 40)).filter(Boolean).join(', ')
      if (k) fakty.push(`Konkurenci z researchu: ${k}`)
    }
    if (raport.luka_rynkowa) fakty.push(`Okno rynkowe: ${s(raport.luka_rynkowa, 240)}`)
  }

  return `Jesteś szefem sprzedaży i marketingu, który wielokrotnie wprowadzał na polski rynek niszowe narzędzia SaaS B2B od zera do pierwszych klientów. Tworzysz konkretny, wykonalny plan zdobycia pierwszych klientów ORAZ gotowe materiały sprzedażowe. Piszesz po polsku, językiem grupy docelowej, zero korpomowy, zero lania wody.
...
Zwróć WYŁĄCZNIE poprawny JSON (bez markdown):
{
  ...
  "pakiet": {
    "reklamy": [
      {"koncept": "nazwa kąta, np. „Oszczędność czasu”", "naglowek": "≤10 słów", "tekst": "2-4 zdania primary text", "cta": "np. Wypróbuj za darmo / Zobacz demo", "wizual_brief": "co na grafice/wideo — konkretnie, dla projektanta", "format": "feed 1:1 | reel 9:16 | karuzela"}
    ],
    "posty": [
...
Wymagania ilościowe: kanaly 4-6, obiekcje 4-5, reklamy DOKŁADNIE 3 (różne kąty), posty 2-3, maile_powitalne 3.`
}
...
function saneGtm(g: any): boolean {
  ...
  if (!k.reklamy.every((r: unknown) => !!r && typeof r === 'object'
    && typeof (r as Record<string, unknown>).naglowek === 'string'
    && typeof (r as Record<string, unknown>).tekst === 'string')) return false
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: session, error: sErr } = await supabase
      .from('spar_sessions')
      .select('id, preview_brief, problem_summary, business_plan, market_report, gtm')
      .eq('id', sessionId)
      .maybeSingle()
    ...
    if (genCount >= MAX_GENERATIONS) {
      if (existing) { const { _meta: _d, ...gtm } = existing; return jsonResponse({ gtm, cached: true }, 200, cors) }
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: OPENAI_MODEL,
        messages: [{ role: 'user', content: buildPrompt(brief, karta, plan, raport) }],
    ...
    if (!res.ok) {
      ...
      console.error('[spar-gtm] openai error:', res.status, errText.slice(0, 500))
    ...
    try {
      ...
      await supabase.from('spar_usage').insert({
        session_id: sessionId, kind: 'gtm', model: OPENAI_MODEL,
    ...
    } catch (uErr) { console.error('[spar-gtm] usage insert error:', uErr) }

    const content = data?.choices?.[0]?.message?.content
    let gtm: Record<string, unknown>
    try { gtm = JSON.parse(content) } catch {
      console.error('[spar-gtm] zły JSON, finish:', data?.choices?.[0]?.finish_reason, String(content).slice(0, 300))
      await supabase.rpc('spar_release_lock', { p_session: sessionId, p_key: 'gtm' })
      return jsonResponse({ error: 'blad_generowania' }, 502, cors)
    }
    if (!saneGtm(gtm)) {
      console.error('[spar-gtm] sanity-check fail:', JSON.stringify(gtm).slice(0, 300))
      await supabase.rpc('spar_release_lock', { p_session: sessionId, p_key: 'gtm' })
      return jsonResponse({ error: 'blad_generowania' }, 502, cors)
    }

    const toSave = { ...gtm, _meta: { gen: genCount + 1, at: new Date().toISOString(), model: OPENAI_MODEL } }
    ...
    await supabase.rpc('spar_release_lock', { p_session: sessionId, p_key: 'gtm' })
    return jsonResponse({ gtm, cached: false }, 200, cors)
  } catch (e) {
    console.error('[spar-gtm] ERROR:', e)
