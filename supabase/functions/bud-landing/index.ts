// bud-landing — generacja DZIAŁAJĄCEGO landinga HTML dla SKLEPU z lejka
// "Zbuduję / Sklep" (AWE — e-commerce fizyczny; tomekniedzwiecki.pl/sklep).
// Następny poziom po statycznych podglądach bud-image: gpt-5.5 pisze kompletny,
// samowystarczalny plik HTML (copy direct-response + design z brief.design +
// cena z business_plan), zapisywany w publicznym Storage → link live do
// oglądania i udostępniania.
//
// ⚠️ DEPLOY: ZAWSZE z flagą --no-verify-jwt:
//   npx supabase functions deploy bud-landing --no-verify-jwt
//
// DOSTĘP:
...
//     bud_usage kind='landing') + 5/dobę/IP — endpoint publiczny, każde
//     wywołanie pali ~$0.45 na gpt-5.5
//   - admin: nagłówek x-admin-secret == BUD_CRON_SECRET omija werdykt
...
// sprawdzasz: HEAD na zwrócony url (200 = plik jest) albo wpis w bud_usage
...
// pokazuje surowy kod. Landing renderuje strona-wrapper na NASZEJ domenie:
// https://tomekniedzwiecki.pl/sklep/podglad/?sid=<uuid>&t=<ts>
// (fetch ze Storage — CORS '*' — i render w sandboxowanym iframe srcdoc).
//
// Koszty: bud_usage kind='landing' (tokeny + cost_usd wg cennika modelu) —
// panel TN Aplikacje liczy z tego PLN jak dla chat/plan/image.
//
// Sekrety:
//   OPENAI_API_KEY      — klucz OpenAI (już ustawiony)
//   BUD_LANDING_MODEL   — opcjonalny override modelu (default gpt-5.5;
//                         jakość > koszt)
//   BUD_CRON_SECRET     — sekret admina (współdzielony z bud-followups)
...
const LANDING_MODEL = Deno.env.get('BUD_LANDING_MODEL') || 'gpt-5.5'
// HTML landinga to 15-25k tokenów + reasoning; 3000 ze bud-chat by ucięło plik
const MAX_COMPLETION_TOKENS = 40000
const MAX_LANDINGS_PER_SESSION = 3
const MAX_LANDINGS_PER_IP_PER_DAY = parseInt(Deno.env.get('BUD_LANDING_IP_DAILY') || '5', 10)
const STORAGE_BUCKET = 'attachments'

// Cennik USD per 1M tokenów (jak w bud-chat) — do logu kosztów w bud_usage
...
// 04-design.md: dokładnie 3 wow momenty hero/mid/conversion) — w kontekście
// landinga SKLEPU e-commerce (produkt fizyczny). Tu reguły e-commerce SĄ na
// miejscu: COD / „płatność przy odbiorze", recenzje/opinie, wysyłka, gwarancja
// zwrotu — to standard sklepu i NIE są zakazane.
let SYSTEM_PROMPT = ''

// ── Pakiet jakości v2: rotacja wariantów + walidacja + krytyk ──
//
// (2) RÓŻNORODNOŚĆ: bez menu wariantów wszystkie landingi zbiegają do jednego
// układu. Losujemy per generacja 3 hero do wyboru (adaptacja banku
// docs/landing/reference/section-variants.md pod sklep e-commerce) — model
// wybiera świadomie pod charakter produktu.
const HERO_VARIANTS: { id: string; opis: string }[] = [
  { id: 'split', opis: 'Split 60/40: lewa kolumna headline+lead+CTA „Kup teraz", prawa duże zdjęcie produktu (placeholder z briefem fotografa) z 1-2 pływającymi elementami (plakietka recenzji, znaczek „darmowa wysyłka").' },
  { id: 'oversized-stat', opis: 'Oversized stat: gigantyczna liczba-korzyść (120px+, np. „4,9/5 z 800 opinii" albo „-30%") jako kotwica hero, headline i opis obok, zdjęcie produktu mniejsze poniżej linii zgięcia.' },
  { id: 'type-led', opis: 'Type-led: typografia wypełnia hero — headline na 2-3 linie wielkim drukiem (clamp do 96px+), jedno słowo wyróżnione (marker/kolor/italic), zdjęcie produktu wjeżdża w drugiej sekcji; pasek zaufania (opinie, COD, zwrot) tuż pod headline.' },
  { id: 'product-first', opis: 'Product-first: duże zdjęcie produktu (edge-to-edge u dołu hero) jako bohater, nad nim krótki headline i CTA wycentrowane; zdjęcie częściowo ścięte linią zgięcia — zachęta do scrolla.' },
  { id: 'before-after', opis: 'Before/After split: hero podzielone na „bez produktu" (problem dnia codziennego, przygaszone) i „z produktem" (rozwiązany problem, żywe kolory, produkt w użyciu); headline spina oba światy.' },
  { id: 'lifestyle-hero', opis: 'Lifestyle-led: hero zbudowane wokół sceny użycia produktu w realnym kontekście (placeholder ze szczegółowym briefem fotografa), headline + CTA na nakładce z czytelnym kontrastem; pasek zaufania pod CTA.' },
]
...
//
// E-COM: COD / „płatność przy odbiorze", recenzje/opinie, wysyłka, gwarancja
// zwrotu są DOZWOLONE (standard sklepu) — NIE blokujemy ich. Czarna lista fraz
// to TYLKO angielszczyzna/placeholdery. Dodatkowo: sandbox-safe (zakaz
// localStorage/sessionStorage w wygenerowanym HTML — wrapper renderuje w
// sandboxowanym iframe srcdoc bez dostępu do storage).
function validateHtml(
  ...
  if (/<img[^>]+src=["']https?:/i.test(html)) issues.push('Strona ładuje zewnętrzne obrazki <img> — usuń, wszystkie wizualizacje mają być zbudowane w HTML/CSS lub placeholderami z briefem.')
  ...
  // Czarna lista fraz: TYLKO angielszczyzna/placeholdery. „Kup teraz", „COD",
  // „płatność przy odbiorze", „opinie", „wysyłka" są poprawne dla sklepu.
  if (/\b(TODO|PLACEHOLDER|Lorem|Get started|Sign up|Learn more|Buy now|Add to cart|Shop now)\b/i.test(html)) {
    issues.push('W treści są angielskie frazy albo placeholdery (TODO/Get started/Sign up/Buy now…) — przepisz na polski.')
  }
  // Sandbox-safe: wrapper renderuje w iframe srcdoc z sandboxem bez storage —
  // localStorage/sessionStorage w wygenerowanym HTML rzucają wyjątek i psują stronę.
  if (/\b(localStorage|sessionStorage)\b/.test(html)) {
    issues.push('Strona używa localStorage/sessionStorage — usuń; podgląd renderuje się w sandboxie bez dostępu do storage (rzuci wyjątek). Stan trzymaj w zmiennych JS.')
  }
  const nazwa = typeof brief.nazwa === 'string' ? brief.nazwa.trim() : ''
  if (nazwa && !html.includes(nazwa)) issues.push(`Nazwa sklepu „${nazwa}" nie pojawia się w treści strony.`)
  const cena = plan && typeof plan.cena === 'number' ? plan.cena : null
  if (cena && !html.includes(String(cena))) {
    issues.push(`Cennik nie zawiera ceny ${cena} zł z planu przychodu — sekcja oferty MUSI używać liczb z planu.`)
  }
  return issues
}
...
function buildCriticUser(
  ...
  if (plan) {
    parts.push(`PLAN PRZYCHODU (liczby do oferty/ceny): ${JSON.stringify({ cena: (plan as Record<string, unknown>).cena, cena_jednostka: (plan as Record<string, unknown>).cena_jednostka, model_przychodu: (plan as Record<string, unknown>).model_przychodu })}`)
  }
  if (issues.length) {
...
async function openaiChat(
  ...
  if (!res.ok) {
    ...
    console.error('[bud-landing] OpenAI error:', res.status, errText.slice(0, 500))
...
function buildUserPrompt(
  ...
  delete briefClean.zmien
  const parts = [
    `BRIEF PROJEKTU (ustalenia z rozmowy z pomysłodawcą — jedyne źródło prawdy o sklepie i produkcie):`,
    JSON.stringify(briefClean, null, 1),
  ]
  if (plan) {
    const p = plan as Record<string, unknown>
    parts.push(
      `PLAN PRZYCHODU (źródło liczb do sekcji oferty — cena i model rozliczenia MUSZĄ się zgadzać):`,
      JSON.stringify({
        cena: p.cena,
...
// Mail „strona gotowa" wysyła bud-followups (kind 'landing_ready') z celowym
// opóźnieniem ≥15 min od timestampu t= w landing_url — followup „coś się dla
// Ciebie zbudowało" zamiast natychmiastowego maila, gdy user jeszcze siedzi
// na stronie.
...
function landingCostUsd(model: string, input: number, cached: number, output: number): number {
  ...
  if (!p) {
    console.warn(`[bud-landing] nieznany cennik modelu ${model} — koszt 0`)
    return 0
  }
...
//
// PIPELINE 2-PASS (pakiet jakości):
...
async function generateAndStore(
  ...
  const releaseLock = async () => {
    const { error } = await supabase.rpc('bud_release_lock', { p_session: sessionId, p_key: 'landing' })
    if (error) console.error('[bud-landing] release lock error:', error)
  }
  try {
    // ── PASS 1: generator ──
    ...
    if (!html1) {
      console.error('[bud-landing] pass1 bez kompletnego HTML, finish:', gen.finish, 'len:', gen.content.length)
      await releaseLock()
      return
    }
    const issues = validateHtml(html1, brief, plan)
    if (issues.length) console.log('[bud-landing] pass1 issues:', JSON.stringify(issues))
    ...
    if (uploadError) {
      console.error('[bud-landing] upload error:', uploadError)
      await releaseLock()
      return
    }
    ...
    if (usageErr) console.error('[bud-landing] usage insert error:', usageErr)

    console.log(`[bud-landing] pass1 OK ${sessionId} ${html1.length}B ${Date.now() - startedAt}ms`)

    // ── PASS 2 w osobnej inwokacji (pełne 400 s dla krytyka) ──
    const SUPABASE_URL = Deno.env.get('SUPABASE_URL')
    const ADMIN_SECRET = Deno.env.get('BUD_CRON_SECRET')
    if (SUPABASE_URL && ADMIN_SECRET) {
      try {
        const trig = await fetch(`${SUPABASE_URL}/functions/v1/bud-landing`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-admin-secret': ADMIN_SECRET },
          body: JSON.stringify({ action: 'critic', sessionId, path: storagePath }),
        })
        if (!trig.ok) console.error('[bud-landing] critic trigger error:', trig.status, await trig.text().catch(() => ''))
      } catch (trigErr) {
        console.error('[bud-landing] critic trigger fetch error:', trigErr)
      }
    }
  } catch (err) {
    console.error('[bud-landing] background ERROR:', err instanceof Error ? err.message : String(err))
    await releaseLock()
  }
}
...
async function runCriticTask(
  ...
  try {
    ...
    if (sErr || !session || !session.preview_brief) {
      console.error('[bud-landing] critic: brak sesji/briefu', sErr)
      return
    }
    ...
    if (dlErr || !file) {
      console.error('[bud-landing] critic: download error', dlErr)
      return
    }
    ...
    if (critic.content) {
      ...
      if (candidate && candidate.length >= html1.length * 0.6) {
        ...
        if (!upErr) {
          ...
          if (issues2.length) console.log('[bud-landing] critic remaining issues:', JSON.stringify(issues2))
        } else {
          console.error('[bud-landing] critic upload error:', upErr)
          criticStatus = 'upload_failed'
        }
      } else {
        console.error('[bud-landing] critic odrzucony (brak HTML albo plik okrojony), len:',
    ...
    const { data: usageRow, error: findErr } = await supabase
      ...
      .maybeSingle()
    if (findErr || !usageRow) {
      console.error('[bud-landing] critic: brak wpisu usage do aktualizacji', findErr)
    } else {
      ...
      const { error: updErr } = await supabase.from('bud_usage').update({
      ...
      if (updErr) console.error('[bud-landing] critic usage update error:', updErr)
    }
    console.log(`[bud-landing] critic ${criticStatus} ${sessionId} ${finalLen}B ${Date.now() - startedAt}ms`)
  } catch (err) {
    console.error('[bud-landing] critic ERROR:', err instanceof Error ? err.message : String(err))
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!OPENAI_API_KEY || !SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !ADMIN_SECRET) {
      console.error('[bud-landing] brak konfiguracji')
    ...
    if (body.action === 'critic') {
      ...
      if (path !== `bud/${sessionId}/` + path.split('/').pop() || !/^landing-\d{10,16}\.html$/.test(path.split('/').pop() || '')) {
    ...
    const { data: session, error: sessionError } = await supabase
      ...
      .maybeSingle()
    if (sessionError) {
      console.error('[bud-landing] session fetch error:', sessionError)
    ...
    // działać jak hasło (lustrzane odbicie bud-chat).
    if (!isAdmin) {
    ...
    const { count: landingCount, error: countErr } = await supabase
      ...
      .eq('kind', 'landing')
    if (countErr) {
      console.error('[bud-landing] usage count error:', countErr)
    ...
    const { data: genLock } = await supabase.rpc('bud_claim_lock', { p_session: sessionId, p_key: 'landing', p_ttl_sec: 480 })
    ...
    // (wzorzec bud-image; liczone z bud_usage.created_at, nie z wieku sesji)
    ...
    if (!isAdmin && ip) {
      ...
      const { data: ipSessions, error: ipErr } = await supabase
        ...
        .eq('ip', ip)
      if (ipErr) {
        console.error('[bud-landing] ip sessions query error:', ipErr)
        ...
        if (ipUsageErr) {
          console.error('[bud-landing] ip usage count error:', ipUsageErr)
    ...
    const ts = Date.now()
    const storagePath = `bud/${sessionId}/landing-${ts}.html`
    const { data: pub } = supabase.storage.from(STORAGE_BUCKET).getPublicUrl(storagePath)
    // Link do OGLĄDANIA — wrapper na naszej domenie (patrz nagłówek pliku)
    const viewUrl = `https://tomekniedzwiecki.pl/sklep/podglad/?sid=${sessionId}&t=${ts}`
    ...
    console.error('[bud-landing] ERROR:', error instanceof Error ? error.message : String(error))
