...
const KNOWHOW_SRC_AI = `[ŹRÓDŁO POMYSŁU: PODSUNĘŁA GO AI] Rozmówca prawdopodobnie NIE jest ekspertem tej branży — nie odpytuj go z wiedzy, której może nie mieć. Najpierw ustal: dlaczego ten pomysł go przekonuje, co go napędza, jakie ma realne możliwości prowadzenia tego (czas, budżet, dostęp do branży/użytkowników do testów), czego się obawia. A gdy wprost sygnalizuje brak wiedzy („nie wiem", „zgaduję", „liczę na Was") — NIE odbijaj pytania z powrotem; SAM podaj 1–2 konkretne przykłady z branży (typowe etapy/rozwiązania) i poproś, żeby wybrał albo skomentował. Twardą wiedzę branżową uzupełnimy researchem.`
const KNOWHOW_SRC_WSPOLNY = `[ŹRÓDŁO POMYSŁU: WSPÓLNE] Rozmówca zna część tematu, część dołożyła AI. Drąż tam, gdzie ma realne doświadczenie; resztę traktuj jak do uzupełnienia researchem. Zadaj też co najmniej jedno pytanie o JEGO wkład i motywację (co sam wnosi, dlaczego akurat ta nisza, czym może realnie pomóc), żeby rozmowa nie zsunęła się w czyste odpytywanie proceduralne.`
