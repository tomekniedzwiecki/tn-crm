// spar-economics — model cenowy + unit economics + symulacja drogi do 50 klientów
// dla lejka „Aplikacja". Zakładka „Czy to się spina" w panelu projektu.
//
// ⚠️ DEPLOY: ZAWSZE z flagą --no-verify-jwt:
//   npx supabase functions deploy spar-economics --no-verify-jwt
//
// POST { sessionId, force? } ->
//   - economics istnieje i !force -> zwraca zapisany (cached)
//   - inaczej: liczy z karty + briefu + business_plan przez OpenAI,
//     zapisuje w spar_sessions.economics, zwraca.
//
// PODZIAŁ ODPOWIEDZIALNOŚCI (ważne): model zwraca WEJŚCIA + uzasadnienia z
// realnych benchmarków (cena, marża, CAC, churn, budżet) oraz cennik (tiery).
// Całą ARYTMETYKĘ — LTV, payback, breakeven, zwrot budowy, symulację miesiąc
// po miesiącu — liczy FRONTEND w JS (suwaki przeliczają natychmiast, liczby
// zawsze spójne; modele są zawodne w arytmetyce). Limit 4 generacji/sesja.
...
function buildPrompt(brief: Record<string, unknown>, karta: Record<string, unknown>, plan: Record<string, unknown> | null): string {
  ...
  if (plan && typeof plan.cena === 'number') {
    fakty.push(`Wstępna cena z planu przychodu: ${plan.cena} ${s(plan.cena_jednostka, 20) || 'zł/mies.'} (${s(plan.model_przychodu, 80)})`)
  }

  return `Jesteś doświadczonym operatorem SaaS i analitykiem unit economics dla polskich nisz. Projektujesz model cenowy i realne wejścia do rachunku opłacalności narzędzia, które dopiero powstaje. Piszesz po polsku, do praktyka — konkretnie, bez korpomowy.
...
function saneEconomics(e: any): boolean {
  ...
  if (!c.tiery.every((t: unknown) => !!t && typeof t === 'object'
    && typeof (t as Record<string, unknown>).nazwa === 'string'
    && typeof (t as Record<string, unknown>).cena === 'number')) return false
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: session, error: sErr } = await supabase
      .from('spar_sessions')
      .select('id, preview_brief, problem_summary, business_plan, economics')
      .eq('id', sessionId)
      .maybeSingle()
    ...
    if (genCount >= MAX_GENERATIONS) {
      if (existing) { const { _meta: _d, ...eco } = existing; return jsonResponse({ economics: eco, cached: true }, 200, cors) }
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: OPENAI_MODEL,
        messages: [{ role: 'user', content: buildPrompt(brief, karta, plan) }],
    ...
    if (!res.ok) {
      ...
      console.error('[spar-economics] openai error:', res.status, errText.slice(0, 500))
    ...
    try {
      ...
      await supabase.from('spar_usage').insert({
        session_id: sessionId, kind: 'economics', model: OPENAI_MODEL,
    ...
    } catch (uErr) { console.error('[spar-economics] usage insert error:', uErr) }

    const content = data?.choices?.[0]?.message?.content
    let eco: Record<string, unknown>
    try { eco = JSON.parse(content) } catch {
      console.error('[spar-economics] zły JSON, finish:', data?.choices?.[0]?.finish_reason, String(content).slice(0, 300))
      await supabase.rpc('spar_release_lock', { p_session: sessionId, p_key: 'economics' })
      return jsonResponse({ error: 'blad_generowania' }, 502, cors)
    }
    if (!saneEconomics(eco)) {
      console.error('[spar-economics] sanity-check fail:', JSON.stringify(eco).slice(0, 300))
      await supabase.rpc('spar_release_lock', { p_session: sessionId, p_key: 'economics' })
      return jsonResponse({ error: 'blad_generowania' }, 502, cors)
    }
    ...
    console.error('[spar-economics] ERROR:', e)
