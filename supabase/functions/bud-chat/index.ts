...
- Koszt budowy: NIE MA jednego cennika i NIE podajesz żadnych kwot budowy. Plan startu (towar, reklamy, budowa) Tomek układa POD KWOTĘ, którą lead może zainwestować — dlatego pytamy o budżet. Na pytanie „ile to kosztuje" odpowiadaj dokładnie tym mechanizmem, ciepło i wprost — NIGDY nie zbywaj samym „to na rozmowie".
...
- PIENIĄDZE: projekt, raport i podgląd sklepu są DARMOWE i zostają jego. ŻADNYCH kwot budowy, procentów ani symulacji NIE podajesz przed rezerwacją — plan startu (towar, reklamy, budowa) Tomek układa POD KWOTĘ, którą lead może zainwestować. Na pytanie o koszty odpowiadaj TYM mechanizmem, wprost i bez zbywania: „nie ma jednego cennika — powiedz, jaką kwotę realnie możesz zainwestować, a Tomek ułoży pod nią plan; szczegółowy podział dostaniesz od niego po rezerwacji". Gdy lead sygnalizuje „mam mało / ostatnie pieniądze / jestem spłukany" — odpowiedz uczciwie i z szacunkiem (realny start to min. ~2 tys. zł), nie ciągnij go pod ścianę, której nie udźwignie.
...
if (!GATE_INSTRUCTION) { try { const { data: __ep } = await supabase.from('settings').select('key, value').in('key', ['budowanie_etap_gate', 'budowanie_etap_kierunki', 'budowanie_etap_preview_po_kierunku', 'budowanie_etap_wspolpraca', 'budowanie_etap_rezygnacja', 'budowanie_model_biznesowy']); const __ev = (k: string) => ((__ep || []) as Array<{ key: string; value: string }>).find((r) => r.key === k)?.value || ''; GATE_INSTRUCTION = __ev('budowanie_etap_gate'); KIERUNKI_INSTRUCTION = __ev('budowanie_etap_kierunki'); PREVIEW_AFTER_GATE_INSTRUCTION = __ev('budowanie_etap_preview_po_kierunku'); COLLAB_PHASE_INSTRUCTION = __ev('budowanie_etap_wspolpraca'); RESIGNATION_INSTRUCTION = __ev('budowanie_etap_rezygnacja'); MODEL_FACTS = __ev('budowanie_model_biznesowy') } catch (_e) { /* fallback poniżej */ } }
      ...
      if (isKnowHowMode) {
        ...
        // „plan pod Twoją kwotę" i zakaz liczb; bot odpowiada o kosztach TYM mechanizmem
        // (wprost, ciepło), zamiast zbywać ALBO zrzucać cennik (obie skrajności zabijały).
        sessionContext += `\n\n[FAKTY OFERTY — gdy rozmówca pyta o koszty, udział Tomka albo warunki współpracy, odpowiadaj WPROST z tego bloku (mechanizm „plan pod Twoją kwotę"); ZERO kwot budowy i ZERO procentu. Dotyczą OFERTY, nie danych z raportu rynku.]\n${MODEL_FACTS}`
