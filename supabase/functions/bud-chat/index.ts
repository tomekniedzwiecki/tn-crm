...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.event === 'knowhow_close') {
      ...
      // (lustro poprawki spar-chat 2026-07-15 — model „zamykał" etap słownie w czacie).
...
if (!GATE_INSTRUCTION) { try { const { data: __ep } = await supabase.from('settings').select('key, value').in('key', ['budowanie_etap_gate', 'budowanie_etap_kierunki', 'budowanie_etap_preview_po_kierunku', 'budowanie_etap_wspolpraca', 'budowanie_etap_rezygnacja', 'budowanie_model_biznesowy']); const __ev = (k: string) => ((__ep || []) as Array<{ key: string; value: string }>).find((r) => r.key === k)?.value || ''; GATE_INSTRUCTION = __ev('budowanie_etap_gate'); KIERUNKI_INSTRUCTION = __ev('budowanie_etap_kierunki'); PREVIEW_AFTER_GATE_INSTRUCTION = __ev('budowanie_etap_preview_po_kierunku'); COLLAB_PHASE_INSTRUCTION = __ev('budowanie_etap_wspolpraca'); RESIGNATION_INSTRUCTION = __ev('budowanie_etap_rezygnacja'); MODEL_FACTS = __ev('budowanie_model_biznesowy') } catch (_e) { /* fallback poniżej */ } }
      ...
      if (isKnowHowMode) {
        ...
        // Twardy guard w kodzie (lustro spar-chat 2026-07-15): model NIE może zamykać
        // etapu słownie — inaczej knowhow_closed_at zostaje NULL, a klient myśli, że koniec.
        else sessionContext += `\n\n[ZAMKNIĘCIE ETAPU = PRZYCISK, NIE CZAT] Etap dopracowania zamyka WYŁĄCZNIE klient przyciskiem „To już wszystko — przejdź do budowy →" na pasku „Etap · Dopracowanie wizji" BEZPOŚREDNIO NAD polem pisania wiadomości (tak go opisuj, gdy klient pyta gdzie kliknąć). Ty NIE możesz zamknąć etapu: NIGDY nie ogłaszaj „etap domknięty", NIE pisz „ruszam z budową", NIE podsuwaj w <opcje> pozycji typu „Domykamy etap". ZAMKNIĘCIE JEST OSTATECZNE: po kliknięciu rozmowa zamyka się na dobre i Tomek rusza z realizacją — NIGDY nie obiecuj, że po zamknięciu klient będzie mógł tu wrócić lub coś dopisać (nowe pomysły omówi przy prezentacji pierwszej wersji). Gdy klient sygnalizuje koniec — podsumuj krótko zebrane, upewnij się, że przekazał wszystko, i skieruj go wprost do tego przycisku.`
        ...
        // (4900 zł z kotwicą wartości), procent udziału nadal po rezerwacji, budżet
