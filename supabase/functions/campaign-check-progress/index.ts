...
async function handleFullManusTask(supabase: any, apiKey: string, supabaseUrl: string, workflowId: string, taskId: string): Promise<boolean> {
  ...
  // NOTA: email i trigger content_ready NIE są wywoływane z crona.
  // Admin musi ręcznie kliknąć "Udostępnij klientowi" — logika w edge function
  // share-content (wywoływana z workflow.html).
  console.log(`[full] Content pipeline complete. Waiting for admin to share with client.`)
