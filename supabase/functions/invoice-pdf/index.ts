...
// Public URL Fakturowni wymaga aktywnej sesji lub wysyłki mailem,
// więc serwer pobiera PDF z api_token (server-side) i streamuje do klienta.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fakturUrl = `https://${subdomain}.fakturownia.pl/invoices/${invoiceId}.pdf?api_token=${apiToken}`
    ...
    if (!pdfResp.ok) {
      ...
      return new Response(JSON.stringify({ error: `Fakturownia ${pdfResp.status}: ${errText.substring(0, 200)}` }), {
        status: pdfResp.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }

    // 4. Stream PDF do klienta
