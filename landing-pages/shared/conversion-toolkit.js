  ...
  const ConversionToolkit = {
    ...
    getDefaultConfig() {
      return {
        ...
        whatsapp: {
          enabled: false,              // wymaga workflowId LUB phone
    ...
    initModules() {
      ...
      if (this.config.whatsapp.enabled || this.config.whatsapp.workflowId) this.initWhatsapp();
    },
    ...
    async initWhatsapp() {
      ...
      // If workflowId provided, fetch from Supabase (anon RLS allows when is_active=TRUE)
      if (cfg.workflowId) {
        try {
          ...
          if (row && row.whatsapp_enabled && row.whatsapp_phone) {
            phone = row.whatsapp_phone;
            enabled = true;
          } else {
            return; // not enabled — bail
          }
        } catch (e) {
          console.warn('[ConversionToolkit] whatsapp workflow fetch failed:', e);
          if (!cfg.phone) return;
        }
      }
